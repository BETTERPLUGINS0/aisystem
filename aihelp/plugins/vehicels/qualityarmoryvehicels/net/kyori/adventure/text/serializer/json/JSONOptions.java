...
public final class JSONOptions {
    ...
    private static final OptionState.Versioned BY_DATA_VERSION = OptionState.versionedOptionState().version(0, builder -> builder.value(EMIT_HOVER_EVENT_TYPE, HoverEventValueMode.LEGACY_ONLY).value(EMIT_RGB, false).value(EMIT_HOVER_SHOW_ENTITY_ID_AS_INT_ARRAY, false).value(VALIDATE_STRICT_EVENTS, false).value(EMIT_DEFAULT_ITEM_HOVER_QUANTITY, false).value(SHOW_ITEM_HOVER_DATA_MODE, ShowItemHoverDataMode.EMIT_LEGACY_NBT)).version(2526, builder -> builder.value(EMIT_HOVER_EVENT_TYPE, HoverEventValueMode.MODERN_ONLY).value(EMIT_RGB, true)).version(3679, builder -> builder.value(EMIT_COMPACT_TEXT_COMPONENT, true).value(EMIT_HOVER_SHOW_ENTITY_ID_AS_INT_ARRAY, true).value(VALIDATE_STRICT_EVENTS, true)).version(3819, builder -> builder.value(EMIT_DEFAULT_ITEM_HOVER_QUANTITY, true).value(SHOW_ITEM_HOVER_DATA_MODE, ShowItemHoverDataMode.EMIT_DATA_COMPONENTS)).build();
