 ...
 */
package me.zombie_striker.qav.hooks.worldguard.implementation.v6.handler;
...
import me.zombie_striker.qav.hooks.worldguard.implementation.v6.WorldGuardImplementation;
...
extends Handler {
    ...
    public void initialize(Player player, Location location, ApplicableRegionSet applicableRegionSet) {
        this.handler.initialize(player, location, this.implementation.wrapRegionSet(Objects.requireNonNull(location.getWorld()), applicableRegionSet));
    }
    ...
    public boolean testMoveTo(Player player, Location location, Location location2, ApplicableRegionSet applicableRegionSet, MoveType moveType) {
        return this.handler.testMoveTo(player, location, location2, this.implementation.wrapRegionSet(Objects.requireNonNull(location2.getWorld()), applicableRegionSet), moveType.name());
    }
    ...
    public boolean onCrossBoundary(Player player, Location location, Location location2, ApplicableRegionSet applicableRegionSet, Set<ProtectedRegion> set, Set<ProtectedRegion> set2, MoveType moveType) {
        ImmutableSet<IWrappedRegion> immutableSet = ImmutableSet.copyOf(Collections2.transform(set, protectedRegion -> new WrappedRegion(location2.getWorld(), (ProtectedRegion)protectedRegion)));
        ImmutableSet<IWrappedRegion> immutableSet2 = ImmutableSet.copyOf(Collections2.transform(set2, protectedRegion -> new WrappedRegion(location.getWorld(), (ProtectedRegion)protectedRegion)));
        return this.handler.onCrossBoundary(player, location, location2, this.implementation.wrapRegionSet(Objects.requireNonNull(location2.getWorld()), applicableRegionSet), immutableSet, immutableSet2, moveType.name());
    }
