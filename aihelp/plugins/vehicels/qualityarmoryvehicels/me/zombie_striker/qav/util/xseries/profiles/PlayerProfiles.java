...
public final class PlayerProfiles {
    ...
    private static final Property XSERIES_GAMEPROFILE_SIGNATURE = new Property("XSeries", "13.0.0");
    ...
    public static void removeTimestamp(GameProfile gameProfile) {
        JsonObject jsonObject = Optional.ofNullable(PlayerProfiles.getTextureValue(gameProfile)).map(PlayerProfiles::decodeBase64).map(string -> new JsonParser().parse((String)string).getAsJsonObject()).orElse(null);
