...
public class MinecartTrackLogic {
   ...
   private void getNeighbours(Rails rails) {
      ...
      if (rails.isOnSlope()) {
         ...
         BlockFace[] var2 = FaceUtil.getFaces(rails.getDirection());
   ...
   private void refreshNeighbours() {
      for(int i = 0; i < this.neigh.size(); ++i) {
         MinecartTrackLogic blockminecarttrackabstract_minecarttracklogic = this.createTrackLogic((IntVector3)this.neigh.get(i));
   ...
   private MinecartTrackLogic createTrackLogic(IntVector3 blockposition) {
      ...
      if (MaterialUtil.ISRAILS.get(iblockdata)) {
         ...
         if (MaterialUtil.ISRAILS.get(iblockdata)) {
            ...
            iblockdata = WorldUtil.getBlockData(this.world, blockposition1);
            return MaterialUtil.ISRAILS.get(iblockdata) ? new MinecartTrackLogic(this.world, blockposition1, iblockdata) : null;
         }
      }
   }
   ...
   private boolean isNeighbourAt(IntVector3 blockposition) {
      for(int i = 0; i < this.neigh.size(); ++i) {
         IntVector3 blockposition1 = (IntVector3)this.neigh.get(i);
   ...
   private boolean isRailsAtUpDown(IntVector3 blockposition) {
      return this.isRailsAt(blockposition) || this.isRailsAt(blockposition.add(BlockFace.UP)) || this.isRailsAt(blockposition.add(BlockFace.DOWN));
   }

   private boolean isRailsAt(IntVector3 pos) {
      return (Boolean)MaterialUtil.ISRAILS.get(this.world, pos.x, pos.y, pos.z);
   }
   ...
   private void c(MinecartTrackLogic blockminecarttrackabstract_minecarttracklogic) {
      ...
      if (railsSet && newRails.getDirection() == BlockFace.SOUTH) {
         if (this.isRailsAt(pos_north.add(BlockFace.UP))) {
            railsSet = true;
            newRails.setDirection(BlockFace.NORTH, true);
         }

         if (this.isRailsAt(pos_south.add(BlockFace.UP))) {
            railsSet = true;
      ...
      if (railsSet && newRails.getDirection() == BlockFace.EAST) {
         if (this.isRailsAt(pos_east.add(BlockFace.UP))) {
            railsSet = true;
            newRails.setDirection(BlockFace.EAST, true);
         }

         if (this.isRailsAt(pos_west.add(BlockFace.UP))) {
            railsSet = true;
