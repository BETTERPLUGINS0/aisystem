...
public class DebugToolUtil {
   ...
   private static final NumberFormat DEFAULT_NUMBER_FORMAT_SMALL;
   private static final NumberFormat DEFAULT_NUMBER_FORMAT_LARGE;
   ...
   public static void showParticle(Location loc) {
      Util.spawnDustParticle(loc, 1.0D, 0.1D, 0.1D);
   }
   ...
   public static void showEndOfTheRail(Player player, TrackWalkingPoint walker, double initialDistance) {
      ...
      if (Math.abs(p.posX - Math.floor(p.posX)) < 0.01D || Math.abs(p.posY - Math.floor(p.posY)) < 0.01D || Math.abs(p.posZ - Math.floor(p.posZ)) < 0.01D) {
         p.move(-0.01D);
      }

      if (walker.failReason == TrackWalkingPoint.FailReason.NAVIGATION_ABORTED) {
         player.sendMessage(ChatColor.RED + "A blocker sign at " + ChatColor.YELLOW + coordinates(p) + ChatColor.RED + " is blocking trains after " + (int)(walker.movedTotal + initialDistance) + " blocks!");
      } else {
         player.sendMessage(ChatColor.RED + "End of the rail at " + coordinates(p) + " after " + (int)(walker.movedTotal + initialDistance) + " blocks!");
      }

   }

   public static String formatNumber(double value) {
      return (value > 10.0D ? DEFAULT_NUMBER_FORMAT_LARGE : DEFAULT_NUMBER_FORMAT_SMALL).format(value);
   }

   static {
      DEFAULT_NUMBER_FORMAT_SMALL = new DecimalFormat("0.0###", DecimalFormatSymbols.getInstance(Locale.ENGLISH));
      DEFAULT_NUMBER_FORMAT_LARGE = new DecimalFormat("0.0", DecimalFormatSymbols.getInstance(Locale.ENGLISH));
      chatcolor_wheel_values = new ChatColor[]{ChatColor.GREEN, ChatColor.BLUE, ChatColor.AQUA, ChatColor.LIGHT_PURPLE, ChatColor.GOLD, ChatColor.DARK_RED, ChatColor.DARK_GREEN, ChatColor.DARK_BLUE, ChatColor.DARK_AQUA, ChatColor.DARK_PURPLE, ChatColor.YELLOW, ChatColor.BLACK, ChatColor.DARK_GRAY, ChatColor.GRAY, ChatColor.WHITE};
      color_wheel_values = new Color[]{Color.GREEN, Color.BLUE, Color.AQUA, Color.FUCHSIA, Color.YELLOW, Color.MAROON, Color.OLIVE, Color.NAVY, Color.TEAL, Color.PURPLE, Color.YELLOW, Color.BLACK, Color.ORANGE, Color.GRAY, Color.WHITE};
   }
}
