...
public class TCConfig {
   ...
   public static double unloadRunawayTrainDistance = 160.0D;
   ...
   public static double spawnSignCooldown = -1.0D;
   public static double itemPickupRadius = 2.0D;
   ...
   public static MapResourcePack resourcePack;
   public static Map<String, Animation> defaultAnimations;
   ...
   public static void load(TrainCarts traincarts, FileConfiguration config) {
      ...
      resourcePack = new MapResourcePack((String)config.get("resourcePack", "server"));
      resourcePack.load();
      ...
      modelSearchCompactFolders = (Boolean)config.get("modelSearchCompactFolders", true);
      if (config.contains("normal")) {
         if (!config.contains("normal.cartDistanceGap") && config.contains("normal.cartDistance")) {
            config.set("normal.cartDistanceGap", (Double)config.get("normal.cartDistance", 1.5D) - 1.0D);
         }
      ...
      cartDistanceGap = (Double)config.get("linkProperties.cartDistanceGap", 0.5D);
      cartDistanceGapMax = (Double)config.get("linkProperties.cartDistanceGapMax", cartDistanceGap + 2.0D);
      cartDistanceForcer = (Double)config.get("linkProperties.cartDistanceForcer", 0.1D);
      cartDistanceForcerConstant = (Double)config.get("linkProperties.cartDistanceForcerConstant", 0.0D);
      ...
      slowDownMultiplierNormal = (Double)config.get("slowDownMultiplier.normal", 0.997D);
      slowDownMultiplierSlow = (Double)config.get("slowDownMultiplier.slow", 0.96D);
      config.setHeader("maxEnterDistance", "\nThe maximum allowed enter radius for enter signs");
      maxEnterDistance = (Double)config.get("maxEnterDistance", 50.0D);
      config.setHeader("maxEjectDistance", "\nThe maximum allowed ejection distance for eject signs");
      maxEjectDistance = (Double)config.get("maxEjectDistance", 10.0D);
      config.setHeader("maxTrainEditdistance", "\nMaximum distance from which /train edit can select trains");
      maxTrainEditdistance = (Double)config.get("maxTrainEditdistance", 64.0D);
      config.setHeader("launchForce", "\nThe amount of velocity stations give when launching trains");
      launchForce = (Double)config.get("launchForce", 10.0D);
      ...
      allowPlayerCollisionFromBehind = (Boolean)config.get("allowPlayerCollisionFromBehind", false);
      config.setHeader("destroyAllOnShutdown", "\nDestroys all existing minecarts on startup and shutdown of the plugin");
      destroyAllOnShutdown = (Boolean)config.get("destroyAllOnShutdown", false);
      if (config.contains("pushAway")) {
         ...
         config.set("collision.pushAwayForce", config.get("pushAway.force", 0.2D));
      ...
      collisionIgnoreOwners = (Boolean)config.get("collision.ignoreOwners", false);
      collisionIgnoreGlobalOwners = (Boolean)config.get("collision.ignoreGlobalOwners", false);
      pushAwayForce = (Double)config.get("collision.pushAwayForce", 0.2D);
      ...
      allMinecartsAreTrainCarts = (Boolean)config.get("allMinecartsAreTrainCarts", false);
      ...
      useCoalFromStorageCart = (Boolean)config.get("useCoalFromStorageCart", false);
      config.setHeader("setOwnerOnPlacement", "\nWhether or not the player that places a minecart is set owner");
      setOwnerOnPlacement = (Boolean)config.get("setOwnerOnPlacement", true);
      ...
      launchFunctionType = (String)config.get("launchFunction", "bezier");
      ...
      keepChunksLoadedOnlyWhenMoving = (Boolean)config.get("keepChunksLoadedOnlyWhenMoving", false);
      ...
      maxKeepChunksLoadedRadius = (Integer)config.get("maxKeepChunksLoadedRadius", 7);
      ...
      enableCeilingBlockCollision = (Boolean)config.get("enableCeilingBlockCollision", true);
      ...
      animationsUseTickTime = (Boolean)config.get("animationsUseTickTime", false);
      ...
      enableSneakingInAttachmentEditor = (Boolean)config.get("enableSneakingInAttachmentEditor", !CommonCapabilities.PLAYER_OFF_HAND);
      ...
      autoSaveInterval = (Integer)config.get("autoSaveInterval", 600);
      ...
      claimNewSavedTrains = (Boolean)config.get("claimNewSavedTrains", true);
      ...
      claimNewSavedModels = (Boolean)config.get("claimNewSavedModels", true);
      ...
      hastebin = new Hastebin(traincarts, (String)config.get("hastebinServer", (String)config.get("hastebinServer", "https://paste.traincarts.net")));
      ...
      enableSeatThirdPersonView = (Boolean)config.get("enableSeatThirdPersonView", false);
      config.setHeader("maxDetectorLength", "\nThe maximum length a detector region (between two detectors) can be");
      maxDetectorLength = (Integer)config.get("maxDetectorLength", 2000);
      ...
      optimizeInteraction = (Boolean)config.get("optimizeInteraction", true);
      ...
      optimizeBlockActivation = (Boolean)config.get("optimizeBlockActivation", true);
      ...
      instantCreativeDestroy = (Boolean)config.get("instantCreativeDestroy", true);
      ...
      allowUpsideDownRails = (Boolean)config.get("allowUpsideDownRails", true);
      ...
      allowSchematicAttachment = (Boolean)config.get("allowSchematicAttachment", true);
      ...
      itemPickupRadius = (Double)config.get("itemPickupRadius", 2.0D);
      ...
      legacySpeedLimiting = (Boolean)config.get("legacySpeedLimiting", false);
      ...
      allowRailEditing = (Boolean)config.get("allowRailEditing", true);
      ...
      manualMovementFactor = (Double)config.get("manualMovementFactor", 0.1D);
      config.setHeader("currencyFormat", "\nThe currency Ticket signs will display in messages, %value% represents the displayed value");
      currencyFormat = (String)config.get("currencyFormat", "%value% Dollars");
      ...
      allowNetherTeleport = (Boolean)config.get("allowNetherTeleport", true);
      ...
      collisionReEnterDelay = (Integer)config.get("collisionReEnterDelay", collisionReEnterDelay);
      ...
      logMutexConflicts = (Boolean)config.get("logMutexConflicts", false);
      ...
      logTrainSplitting = (Boolean)config.get("logTrainSplitting", false);
      ...
      logSyncChunkLoads = (Boolean)config.get("logSyncChunkLoads", false);
      ...
      cacheVerificationTicks = (Integer)cacheConfig.get("verificationTicks", 0);
      ...
      cacheExpireTicks = (Integer)cacheConfig.get("expireTicks", 20);
      ...
      if (config.contains("allowedBlockBreakTypes")) {
         var10 = config.getList("allowedBlockBreakTypes", String.class).iterator();

         while(var10.hasNext()) {
            world = (String)var10.next();
            mat = ParseUtil.parseMaterial(world, (Material)null);
            if (mat != null) {
               allowedBlockBreakTypes.add(mat);
            }
         }
      } else {
      ...
      activatorEjectEnabled = (Boolean)config.get("activatorEjectEnabled", true);
      ...
      enableVanillaActionSigns = (Boolean)config.get("enableVanillaActionSigns", true);
      ...
      var10 = config.getList("enabledWorlds", String.class).iterator();
      ...
      var10 = config.getList("disabledWorlds", String.class).iterator();
      ...
      types.clear();
      Iterator var12 = allowedBlockBreakTypes.iterator();

      while(var12.hasNext()) {
         mat = (Material)var12.next();
      ...
      showTransferAnimations = (Boolean)config.get("showTransferAnimations", true);
      ...
      craftingRequireWorkbench = (Boolean)config.get("craftingRequireWorkbench", true);
      ...
      ArrivalSigns.setTimeDurationFormat((String)config.get("triggerTimerDateFormat", "HH:mm:ss"));
      ...
      if (config.contains("parseOldSigns")) {
         parseOldSigns = (Boolean)config.get("parseOldSigns", false);
      }
      ...
      allowParenthesesFormat = (Boolean)config.get("parseParentheses", true);
      if (config.contains("onlyPoweredEmptySwitchersDoPathfinding")) {
         if ((Boolean)config.get("onlyPoweredEmptySwitchersDoPathfinding", false)) {
      ...
      onlyPoweredSwitchersDoPathFinding = (Boolean)config.get("onlyPoweredSwitchersDoPathFinding", true);
      ...
      onlyEmptySwitchersDoPathFinding = (Boolean)config.get("onlyEmptySwitchersDoPathFinding", false);
      ...
      onlyRegisteredSignsHandleRedstone = (Boolean)config.get("onlyRegisteredSignsHandleRedstone", true);
      config.setHeader("rerouteOnStartup", "\nWhen enabled, re-calculates all path finding routes on plugin startup");
      rerouteOnStartup = (Boolean)config.get("rerouteOnStartup", false);
      ...
      traincarts.getPathProvider().setMaxProcessingPerTick((Integer)config.get("pathFindingMaxProcessingPerTick", 30));
      ...
      switcherResetCountersOnFirstCart = (Boolean)config.get("switcherResetCountersOnFirstCart", true);
      ...
      trainsCheckSignFacing = (Boolean)config.get("trainsCheckSignFacing", true);
      parsers.clear();
      ...
      ConfigurationNode cartLimits;
      if (!config.isNode("defaultAnimations")) {
         Animation[] defaults = new Animation[]{new Animation("rotate", new String[]{"t=0.25 yaw=0.0", "t=0.25 yaw=90.0", "t=0.25 yaw=180.0", "t=0.25 yaw=270.0", "t=0.0 yaw=360.0"}), new Animation("roll", new String[]{"t=0.25 roll=0.0", "t=0.25 roll=90.0", "t=0.25 roll=180.0", "t=0.25 roll=270.0", "t=0.0 roll=360.0"}), new Animation("pitch", new String[]{"t=0.25 pitch=0.0", "t=0.25 pitch=90.0", "t=0.25 pitch=180.0", "t=0.25 pitch=270.0", "t=0.0 pitch=360.0"})};
         cartLimits = config.getNode("defaultAnimations");
         Animation[] var6 = defaults;
         int var7 = defaults.length;

         for(int var8 = 0; var8 < var7; ++var8) {
            Animation anim = var6[var8];
            anim.saveToParentConfig(cartLimits);
         }
      }

      Iterator var15 = config.getNode("defaultAnimations").getNodes().iterator();

      while(var15.hasNext()) {
         cartLimits = (ConfigurationNode)var15.next();
         Animation defaultAnimation = Animation.loadFromConfig(cartLimits);
         defaultAnimations.put(defaultAnimation.getOptions().getName(), defaultAnimation);
      }

      var15 = itemshort.getValues(String.class).entrySet().iterator();
      ...
      allowExternalTicketImagePaths = (Boolean)config.get("allowExternalTicketImagePaths", false);
      ...
      playHissWhenStopAtStation = (Boolean)soundsConfig.get("hissWhenStopAtStation", true);
      ...
      playHissWhenDestroyedBySign = (Boolean)soundsConfig.get("hissWhenDestroyedBySign", true);
      ...
      playHissWhenCartRemoved = (Boolean)soundsConfig.get("playHissWhenCartRemoved", true);
      soundsConfig.setHeader("hissWhenLinked", "Enable/disable hiss sound played when two carts connect together");
      playHissWhenLinked = (Boolean)soundsConfig.get("hissWhenLinked", true);
      ...
      upsideDownSupportedByAll = (Boolean)config.get("upsideDownSupportedByAll", true);
      ...
      if (!config.contains("attachmentTransformParallelism")) {
         ...
         attachmentTransformParallelism = (Integer)config.get("attachmentTransformParallelism", -1);
      }
      ...
      unloadRunawayTrainDistance = (Double)config.get("unloadRunawayTrainDistance", 160.0D);
      ...
      maxCommandSelectorValues = (Integer)config.get("maxCommandSelectorValues", 128);
      ...
      int max = (Integer)config.get("maxConcurrentEffectLoops", 20);
      if (max < 0) {
      ...
      spawnSignCooldown = (Double)config.get("spawnSignCooldown", -1.0D);
      cartLimits = config.getNode("cartLimits");
      ...
      maxCartsPerWorld = (Integer)cartLimits.get("maxCartsPerWorld", -1);
      cartLimits.setHeader("countUnloaded", "\nWhether to include unloaded trains/carts in the maxCartsPerWorld limit");
      maxCartsPerWorldCountUnloaded = (Boolean)cartLimits.get("countUnloaded", false);
      ...
      maxCartsPerTrain = (Integer)cartLimits.get("maxCartsPerTrain", -1);
   }
   ...
   static {
      resourcePack = MapResourcePack.SERVER;
