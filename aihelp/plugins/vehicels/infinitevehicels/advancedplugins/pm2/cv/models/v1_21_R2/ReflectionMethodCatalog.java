...
public enum ReflectionMethodCatalog implements ReflectionUtils.MethodEnum {
   ...
   private ReflectionMethodCatalog(Class<?> target, String obfuscated, String mapped, Class<?>... parameterClasses) {
      this.target = var3;
   ...
   public boolean matchesSignature(String methodName, Class<?>... params) {
   ...
   public boolean belongsToClass(Class<?> clazz) {
      return this.target.equals(var1);
   }

   public static List<ReflectionMethodCatalog> getMethodsForClass(Class<?> clazz) {
      return (List)Arrays.stream(values()).filter((var1) -> {
         return var1.belongsToClass(var0);
      }).collect(Collectors.toList());
   }

   public static ReflectionMethodCatalog findByMappedName(String mappedName) {
