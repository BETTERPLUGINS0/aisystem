package advancedplugins.pm2.cv.models.v1_21_R2.network;
...
import advancedplugins.pm2.cv.models.v1_21_R2.network.patch.InteractionPacketWrapper;
import advancedplugins.pm2.cv.models.v1_21_R2.network.utils.PacketInterceptor;
...
public class ModelPacketHandler extends ChannelDuplexHandler {
   ...
   public ModelPacketHandler(Player player, PipelineWrapper wrapper) {
   ...
   private Packet processOutgoingPacket(Packet packet) {
   ...
   private ClientboundBundlePacket processBundledPackets(ClientboundBundlePacket bundle) {
   ...
   private Packet processSinglePacket(Packet packet) {
   ...
   private PacketPlayOutSpawnEntity processEntitySpawn(PacketPlayOutSpawnEntity packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private List<Packet<? super PacketListenerPlayOut>> postProcessEntitySpawn(PacketPlayOutSpawnEntity packet) {
      return this.generateMountPackets(var1.e());
   }

   private List<Packet<? super PacketListenerPlayOut>> generateMountPackets(UUID entityUuid) {
   ...
   private PacketPlayOutEntityDestroy processEntityRemoval(PacketPlayOutEntityDestroy packet) {
   ...
   private <T extends Packet<? super PacketListenerPlayOut>> T filterByEntityId(T packet) {
      PacketDataSerializer var2 = PacketTransmissionUtility.decodeOutgoingPacket(var1);
      int var3 = var2.l();
      return this.isEntityVisible(var3) ? var1 : null;
   }

   private PacketPlayOutEntityVelocity processVelocityUpdate(PacketPlayOutEntityVelocity packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private PacketPlayOutEntityTeleport processTeleportation(PacketPlayOutEntityTeleport packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private PacketPlayOutAnimation processAnimation(PacketPlayOutAnimation packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private PacketPlayOutEntityMetadata processMetadataUpdate(PacketPlayOutEntityMetadata packet) {
   ...
   private PacketPlayOutEntityMetadata createInvisibilityPacket(PacketPlayOutEntityMetadata original) {
      ArrayList var2 = new ArrayList();
   ...
   private PacketPlayOutEntityEquipment processEquipmentChange(PacketPlayOutEntityEquipment packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private PacketPlayOutRemoveEntityEffect processEffectRemoval(PacketPlayOutRemoveEntityEffect packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private PacketPlayOutEntityEffect processEffectApplication(PacketPlayOutEntityEffect packet) {
      return this.isEntityVisible(var1.b()) ? var1 : null;
   }

   private boolean isEntityVisible(int entityId) {
   ...
   private ClientboundKeepAlivePacket processKeepAlive(ClientboundKeepAlivePacket packet) {
   ...
   private Packet<? super PacketListenerPlayIn> processInteraction(PacketPlayInUseEntity packet) {
   ...
   private Packet<? super PacketListenerPlayIn> resolveInteractionTarget(int targetId, int actionType, PacketPlayInUseEntity original) {
   ...
   private PacketPlayInSteerVehicle processPlayerControls(PacketPlayInSteerVehicle packet) {
   ...
   private void updateMountInput(MountController controller, PacketPlayInSteerVehicle packet) {
   ...
   private ServerboundPongPacket processPongResponse(ServerboundPongPacket packet) {
      this.syncMonitor.recordPongTime(System.currentTimeMillis());
      return null;
   }

   private PacketPlayInFlying processMovement(PacketPlayInFlying packet) {
   ...
   private ServerboundClientTickEndPacket processTickEnd(ServerboundClientTickEndPacket packet) {
      if (ConfigProperty.SYNC_CLIENT_TICK_END.getBoolean()) {
         this.syncMonitor.recordClientSyncTime(System.currentTimeMillis());
      }
