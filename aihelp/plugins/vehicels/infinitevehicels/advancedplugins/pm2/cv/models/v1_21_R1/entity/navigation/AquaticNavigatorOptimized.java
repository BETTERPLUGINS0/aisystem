...
public class AquaticNavigatorOptimized extends NavigationGuardian {
   ...
   public AquaticNavigatorOptimized(EntityInsentient mob) {
      super(var1, var1.dO());
      this.swimProcessor = new AquaticNavigatorOptimized.SwimPathProcessor(var1);
   }

   public boolean a(@Nullable PathEntity swimPath, double swimSpeed) {
   ...
   private class SwimPathProcessor {
      ...
      SwimPathProcessor(EntityInsentient swimmer) {
         this.swimmer = var2;
      }

      void optimizeSwimPath(PathEntity waterPath) {
         int var2 = var1.f();
      ...
      private double measureDistanceToPoint(Vec3D pointLocation) {
         double var2 = var1.c - this.swimmer.dt();
         double var4 = var1.d - this.swimmer.dv();
         double var6 = var1.e - this.swimmer.dz();
         return var2 * var2 + var4 * var4 + var6 * var6;
      }

      private float calculateSwimReach() {
         float var1 = this.swimmer.dj();
         return var1 > 0.75F ? var1 / 2.0F : 0.75F - var1 / 2.0F;
      }

      private boolean isWithinReach(Vec3D position, Vec3D target, float reach) {
         double var4 = Math.abs(this.swimmer.dt() - var2.c);
         double var6 = Math.abs(this.swimmer.dv() - var2.d);
         double var8 = Math.abs(this.swimmer.dz() - var2.e);
         return var4 < (double)var3 && var8 < (double)var3 && var6 < 1.0D;
      }

      private boolean canMoveToNextPoint(Vec3D position) {
      ...
      private boolean shouldSwimToNext(Vec3D currentSpot, PathEntity swimPath) {
         if (var2.f() + 1 >= var2.e()) {
            return false;
         } else {
