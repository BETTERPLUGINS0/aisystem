...
public class EntityTrackingWrapper implements TrackedEntity {
   ...
   public EntityTrackingWrapper(Entity entity, Supplier<EntityTracker> provider, @NotNull EntityTracker initialTracker) {
   ...
   private Set<UUID> collectVisiblePlayers(Predicate<Player> additionalFilter) {
   ...
   private boolean isPlayerVisible(Player player, Predicate<Player> additionalFilter) {
   ...
   private void synchronizeTrackerData(EntityTracker newTracker) {
   ...
   @Generated
   public void setCachedTracker(@NotNull EntityTracker cachedTracker) {
      this.cachedTracker = var1;
   }
}
