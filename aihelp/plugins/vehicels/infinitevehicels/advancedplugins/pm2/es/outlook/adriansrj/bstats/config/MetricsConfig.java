package es.outlook.adriansrj.bstats.config;
...
public class MetricsConfig {
   ...
   private void readConfig() {
      ...
      if (var1 == null) {
         ...
         this.enabled = (Boolean)this.getConfigValue("enabled", var1).map("true"::equals).orElse(true);
         this.serverUUID = (String)this.getConfigValue("server-uuid", var1).orElse((Object)null);
         this.logErrors = (Boolean)this.getConfigValue("log-errors", var1).map("true"::equals).orElse(false);
         this.logSentData = (Boolean)this.getConfigValue("log-sent-data", var1).map("true"::equals).orElse(false);
         this.logResponseStatusText = (Boolean)this.getConfigValue("log-response-status-text", var1).map("true"::equals).orElse(false);
      }
   }
   ...
   private Optional<String> getConfigValue(String var1, List<String> var2) {
      return var2.stream().filter((var1x) -> {
         ...
         return var1x.replaceFirst(Pattern.quote(var1 + "="), "");
      }).findFirst();
   }
   ...
   private List<String> readFile(File var1) {
      if (!var1.exists()) {
         ...
         try {
            ...
            try {
               var4 = (List)var3.lines().collect(Collectors.toList());
