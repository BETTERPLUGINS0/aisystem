...
extends MTVSubCommand {
    ...
    public boolean execute() {
        ...
        this.sendMessage(ConfigModule.messagesConfig.getMessage(Message.VEHICLE_INFO_SPEED) + formatter.format(vehicle.getMaxSpeed() * 20.0).toString().replace(",", ".") + " blocks/sec");
        this.sendMessage(ConfigModule.messagesConfig.getMessage(Message.VEHICLE_INFO_ACCELERATION) + formatter.format(vehicle.getAccelerationSpeed() / 0.2 * 100.0).toString().replace(",", ".") + " blocks/sec^2");
        this.sendMessage(ConfigModule.messagesConfig.getMessage(Message.VEHICLE_INFO_HEALTH) + (double)Math.round(vehicle.getHealth() * 100.0) / 100.0);
