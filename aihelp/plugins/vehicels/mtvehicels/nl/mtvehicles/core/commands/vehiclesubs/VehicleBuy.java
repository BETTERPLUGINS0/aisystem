...
extends MTVSubCommand {
    ...
    public boolean execute() {
        ...
        if (this.arguments.length != 2 && this.arguments.length != 3) {
        ...
        String carUuid = vehicleList.get(this.arguments[1]);
        Double price = VehicleUtils.getPrice(carUuid);
        if (price == null) {
            this.sender.sendMessage(ConfigModule.messagesConfig.getMessage(Message.GIVE_CAR_NOT_FOUND));
            return true;
        }
        boolean useVoucher = this.arguments.length < 3 ? false : this.arguments[2].equals("--voucher:true");
        if (useVoucher) {
