...
@Name(value="Add rider/member to MTV vehicle")
@Description(value={"Add a rider or member to an MTV vehicle."})
@Examples(value={"add {_player} as a rider of the vehicle {_car}", "add player {_offlinePlayer} as a member to mtv vehicle {_car}"})
@Since(value={"2.5.5"})
public class EffAddRiderMember
extends Effect {
    ...
    static {
        Skript.registerEffect(EffAddRiderMember.class, (String[])new String[]{"add [player] %offlineplayer% as [a] rider (of|to) [the] [mtv] vehicle %mtvehicle%", "add [player] %offlineplayer% as [a] member (of|to) [the] [mtv] vehicle %mtvehicle%"});
    }
