...
final class ACFPatterns {
    ...
    static final Map<String, Pattern> patternCache = ExpiringMap.builder().maxSize(200).expiration(1L, TimeUnit.HOURS).expirationPolicy(ExpirationPolicy.ACCESSED).build();

    private ACFPatterns() {
    }

    public static Pattern getPattern(String string) {
        return patternCache.computeIfAbsent(string, string2 -> Pattern.compile(string));
    }
}
