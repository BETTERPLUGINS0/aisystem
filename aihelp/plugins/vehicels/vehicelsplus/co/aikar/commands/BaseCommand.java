...
public abstract class BaseCommand {
    ...
    private void onRegister(CommandManager commandManager, String string) {
        ...
        if (stringArray != null) {
            ...
            for (String string2 : hashSet) {
                this.registerSubclasses(string2);
        ...
        for (Class<?> clazz : this.getClass().getDeclaredClasses()) {
            ...
            try {
                ...
                for (Constructor<?> constructor : constructorArray) {
                    ...
                    if (parameterArray.length == 1 && parameterArray[0].getType().isAssignableFrom(this.getClass())) {
                        baseCommand = (BaseCommand)constructor.newInstance(this);
                        continue;
                    }
                    arrayList.add("Found unusable constructor: " + constructor.getName() + "(" + Stream.of(parameterArray).map(parameter -> parameter.getType().getSimpleName() + " " + parameter.getName()).collect(Collectors.joining("<c2>,</c2> ")) + ")");
                }
                if (baseCommand != null) {
                ...
                for (String string2 : arrayList) {
                    this.manager.log(LogLevel.INFO, string2);
                }
    ...
    private void registerSubcommand(Method method, String string) {
        ...
        for (String string4 : set) {
            this.subCommands.put(string4, registeredCommand);
        }
        ...
        try {
            ...
            if (routeSearch != null) {
                for (RegisteredCommand registeredCommand : routeSearch.commands) {
                    if (registeredCommand.scope != this) continue;
    ...
    private boolean checkPrecommand(CommandOperationContext commandOperationContext, RegisteredCommand registeredCommand, CommandIssuer commandIssuer, String[] stringArray) {
        ...
        if (method != null) {
            try {
                ...
                return (Boolean)method.invoke(this, objectArray);
