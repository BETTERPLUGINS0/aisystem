...
extends DefaultVehicleModel {
    ...
    public VehicleModel build() {
        ...
        return VehicleModel.builder().id("ExampleTank").displayName("&cExample &aTank").typeId("tanks").part(new Skin(0.0, 0.0, 0.0, new ItemBuilder(XMaterial.LEATHER_BOOTS.parseItem()).customModelData(7, itemBuilder -> itemBuilder.durability(7).unbreakable()).armorColor(Color.WHITE).getItemStack(), HolderItemPosition.HEAD)).part(new Seat(1.0, -1.0, 0.0, true)).part(new TurretSeat(-3.05, 0.0, 0.9, turret.getIdentifier())).part(turret).permissions(new Permissions("ExampleTank")).sounds(this.defaultSounds).availableColors(ColorList.of(Color.RED, Color.BLACK, Color.WHITE, Color.GRAY, Color.SILVER, Color.BLUE)).maxSpeed(new UpgradableSetting(100, 200, 5, 1000.0, "km/h")).fuelTank(new UpgradableSetting(50, 100, 5, 1000.0, "L")).turningRadius(new UpgradableSetting(7, 15, 1, 1000.0, "")).acceleration(new UpgradableSetting(50, 100, 5, 1000.0, "")).exhaust(new Exhaust(true, -5.0, 0.0, 0.0, XParticle.LARGE_SMOKE.get())).horn(new Horn(true, new Sounds.Sound(XSound.BLOCK_NOTE_BLOCK_BASS.name(), 3))).drift(false).exitWhileMoving(true).price(100000.0).fuel(new Fuel("gasoline", 6.0)).health(5000).trunkSize(27).hitbox(new Hitbox(3.0, 2.0, 1.0)).gearbox(new Gearbox(true)).build();
    }
}
