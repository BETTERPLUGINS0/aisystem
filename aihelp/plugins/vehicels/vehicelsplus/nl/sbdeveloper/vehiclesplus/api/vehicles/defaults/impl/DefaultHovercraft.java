...
public class DefaultHovercraft
extends DefaultVehicleModel {
    @Override
    public VehicleModel build() {
        return VehicleModel.builder().id("ExampleHovercraft").displayName("&cExample &aHovercraft").typeId("hovercrafts").part(new Skin(0.0, 1.0, 0.0, new ItemBuilder(XMaterial.LEATHER_BOOTS.parseItem()).customModelData(5, itemBuilder -> itemBuilder.durability(5).unbreakable()).armorColor(Color.WHITE).getItemStack(), HolderItemPosition.HEAD)).part(new Seat(0.15, -0.3, 0.0, true)).part(new Seat(-0.9, -0.3, 0.8, 90, false)).part(new Seat(-0.9, -0.3, -0.8, -90, false)).permissions(new Permissions("ExampleHovercraft")).sounds(this.defaultSounds).availableColors(ColorList.of(Color.RED, Color.BLACK, Color.WHITE, Color.GRAY, Color.SILVER, Color.BLUE)).maxSpeed(new UpgradableSetting(100, 200, 5, 1000.0, "km/h")).fuelTank(new UpgradableSetting(50, 100, 5, 1000.0, "L")).turningRadius(new UpgradableSetting(5, 10, 1, 1000.0, "")).acceleration(new UpgradableSetting(50, 100, 1, 1000.0, "")).exhaust(new Exhaust(true, -4.0, 0.0, 0.0, XParticle.LARGE_SMOKE.get())).horn(new Horn(true, new Sounds.Sound(XSound.BLOCK_NOTE_BLOCK_BASS.name(), 3))).drift(false).exitWhileMoving(true).price(100000.0).fuel(new Fuel("gasoline", 6.0)).health(100).trunkSize(0).hitbox(new Hitbox(4.0, 2.0, 1.0)).gearbox(new Gearbox(false)).build();
    }
}
