...
public enum ReflectionMethod {
    ...
    private ReflectionMethod(ClassWrapper classWrapper, Class<?>[] classArray, MinecraftVersion minecraftVersion, MinecraftVersion minecraftVersion2, Since ... sinceArray) {
        ...
        boolean bl2 = bl = MinecraftVersion.isForgePresent() && this.name().equals("COMPOUND_MERGE") && MinecraftVersion.getVersion() == MinecraftVersion.MC1_7_R4;
        if (!bl && (!MinecraftVersion.isAtLeastVersion(minecraftVersion) || this.removedAfter != null && MinecraftVersion.isNewerThan(minecraftVersion2))) {
            return;
        }
        ...
        try {
            ...
            if (MinecraftVersion.isForgePresent() && MinecraftVersion.getVersion() == MinecraftVersion.MC1_7_R4) {
                String string3 = Forge1710Mappings.getMethodMapping().getOrDefault(this.name(), string2);
            } else if (this.targetVersion.version.isMojangMapping()) {
                try {
                    String string4 = this.targetVersion.name.split("\\(")[0];
                    this.method = classWrapper.getClazz().getMethod(string4, classArray);
                    this.method.setAccessible(true);
                    this.loaded = true;
                    this.methodName = string4;
                    return;
                } catch (NoSuchMethodException noSuchMethodException) {
                    String string5 = MojangToMapping.getMapping().getOrDefault(classWrapper.getMojangName() + "#" + this.targetVersion.name, "Unmapped" + this.targetVersion.name);
                }
            }
            this.method = classWrapper.getClazz().getDeclaredMethod((String)var11_15, classArray);
            this.method.setAccessible(true);
            this.loaded = true;
            ...
            try {
                ...
                if (this.targetVersion.version.isMojangMapping()) {
                    String string6 = MojangToMapping.getMapping().getOrDefault(classWrapper.getMojangName() + "#" + this.targetVersion.name, "Unmapped" + this.targetVersion.name);
                }
                this.method = classWrapper.getClazz().getMethod((String)var11_17, classArray);
                this.method.setAccessible(true);
                this.loaded = true;
                this.methodName = this.targetVersion.name;
            } catch (NoSuchMethodException | NullPointerException | SecurityException exception2) {
                MinecraftVersion.getLogger().warning("[NBTAPI] Unable to find the method '" + string2 + "' in '" + (classWrapper.getClazz() == null ? classWrapper.getMojangName() : classWrapper.getClazz().getSimpleName()) + "' Args: " + Arrays.toString(classArray) + " Enum: " + (Object)((Object)this));
            }
        }
    }
    ...
    public Object run(Object object2, Object ... objectArray) {
        if (this.method == null) {
            throw new NbtApiException("Method not loaded! '" + (Object)((Object)this) + "'");
        }
        try {
            return this.method.invoke(object2, objectArray);
        } catch (Exception exception) {
            throw new NbtApiException("Error while calling the method '" + this.methodName + "', loaded: " + this.loaded + ", Enum: " + (Object)((Object)this) + ", Passed Class: " + (object2 == null ? "null" : object2.getClass()) + " Args: " + (objectArray == null ? "null" : Arrays.toString(objectArray)) + " Classes: " + (objectArray == null ? "null" : Arrays.asList(objectArray).stream().map(object -> object == null ? "NULL" : object.getClass().getName()).collect(Collectors.toList())), exception);
        }
    }
