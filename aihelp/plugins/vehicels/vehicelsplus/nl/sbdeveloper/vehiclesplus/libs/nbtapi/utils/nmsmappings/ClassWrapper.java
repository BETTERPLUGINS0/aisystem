...
import nl.sbdeveloper.vehiclesplus.libs.nbtapi.utils.nmsmappings.PackageWrapper;
...
public enum ClassWrapper {
    ...
    private ClassWrapper(PackageWrapper packageWrapper, String string2, MinecraftVersion minecraftVersion, MinecraftVersion minecraftVersion2, String string3, String string4) {
        ...
        this.enabled = true;
        try {
            if (MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_18_R1) && string4 != null) {
                try {
            ...
            if (MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_17_R1) && string3 != null) {
                ...
                string2 = Forge1710Mappings.getClassMappings().get(this.name());
                this.clazz = Class.forName(string2);
                ...
                this.clazz = Class.forName(packageWrapper.getUri() + "." + string5 + "." + string2);
            }
        } catch (Throwable throwable) {
            MinecraftVersion.getLogger().log(Level.WARNING, "[NBTAPI] Error while trying to resolve the class '" + string2 + "'!", throwable);
        }
    }
