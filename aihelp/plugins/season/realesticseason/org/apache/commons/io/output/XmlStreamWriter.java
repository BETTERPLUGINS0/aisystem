...
public class XmlStreamWriter extends Writer {
   ...
   public XmlStreamWriter(File var1) {
      this((File)var1, (String)null);
   }

   public XmlStreamWriter(File var1, String var2) {
   ...
   private void detectEncoding(char[] var1, int var2, int var3) {
      int var4 = var3;
      StringBuffer var5 = this.xmlPrologWriter.getBuffer();
      if (var5.length() + var3 > 8192) {
         var4 = 8192 - var5.length();
      }
      ...
      if (var5.length() >= 5) {
         if (var5.substring(0, 5).equals("<?xml")) {
            ...
            if (var6 > 0) {
               ...
               if (var7.find()) {
                  this.encoding = var7.group(1).toUpperCase(Locale.ROOT);
                  this.encoding = this.encoding.substring(1, this.encoding.length() - 1);
               } else {
                  this.encoding = this.defaultEncoding;
               }
            } else if (var5.length() >= 8192) {
               this.encoding = this.defaultEncoding;
            }
         } else {
            this.encoding = this.defaultEncoding;
