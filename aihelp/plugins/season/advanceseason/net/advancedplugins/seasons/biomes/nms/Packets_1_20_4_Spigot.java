...
public class Packets_1_20_4_Spigot
implements IProtocolLib {
    private final INMSBiome nmsBiome;
    private final BiomesHandler biomesHandler;

    public Packets_1_20_4_Spigot(JavaPlugin javaPlugin, BiomesHandler biomesHandler, INMSBiome iNMSBiome) {
        ...
        ProtocolLibrary.getProtocolManager().addPacketListener((PacketListener)new PacketAdapter((Plugin)javaPlugin, ListenerPriority.NORMAL, new PacketType[]{PacketType.Play.Server.MAP_CHUNK}){
            ...
            public void onPacketSending(PacketEvent packetEvent) {
                ...
                ChunkSection[] chunkSectionArray = Packets_1_20_4_Spigot.this.getChunkSections(clientboundLevelChunkPacketData, player.getWorld());
                for (ChunkSection chunkSection : chunkSectionArray) {
                    for (int i = 0; i < 4; ++i) {
                        for (int j = 0; j < 4; ++j) {
                            for (int k = 0; k < 4; ++k) {
                                Integer n = Packets_1_20_4_Spigot.this.getBiome(Packets_1_20_4_Spigot.this.nmsBiome.getBiomeId((BiomeBase)chunkSection.c(i, j, k).a()), season, bl);
                                if (n == null) continue;
                                chunkSection.setBiome(i, j, k, Holder.a((Object)((BiomeBase)NMSBiome_1_20_4_Spigot.registrywritable.a(n.intValue()))));
                            }
                        }
                    }
                }
                try {
                    ...
                    field.set(clientboundLevelChunkPacketData, Packets_1_20_4_Spigot.this.convertToBytes(chunkSectionArray));
    ...
    public ChunkSection[] getChunkSections(ClientboundLevelChunkPacketData clientboundLevelChunkPacketData, World world) {
        ...
        ChunkSection[] chunkSectionArray = new ChunkSection[((CraftWorld)world).getHandle().an()];
        for (int i = 0; i < chunkSectionArray.length; ++i) {
            ChunkSection chunkSection = new ChunkSection(NMSBiome_1_20_4_Spigot.registrywritable);
