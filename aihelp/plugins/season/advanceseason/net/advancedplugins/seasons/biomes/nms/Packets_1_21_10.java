...
public class Packets_1_21_10
implements IProtocolLib {
    private final INMSBiome nmsBiome;
    private final BiomesHandler biomesHandler;

    public Packets_1_21_10(JavaPlugin javaPlugin, BiomesHandler biomesHandler, INMSBiome iNMSBiome) {
        ...
        ProtocolLibrary.getProtocolManager().addPacketListener((PacketListener)new PacketAdapter((Plugin)javaPlugin, ListenerPriority.NORMAL, new PacketType[]{PacketType.Play.Server.MAP_CHUNK}){
            ...
            public void onPacketSending(PacketEvent packetEvent) {
                ...
                ChunkSection[] chunkSectionArray = Packets_1_21_10.this.getChunkSections(clientboundLevelChunkPacketData, player.getWorld());
                for (ChunkSection chunkSection : chunkSectionArray) {
                    for (int i = 0; i < 4; ++i) {
                        for (int j = 0; j < 4; ++j) {
                            for (int k = 0; k < 4; ++k) {
                                Integer n = Packets_1_21_10.this.getBiome(Packets_1_21_10.this.nmsBiome.getBiomeId((BiomeBase)chunkSection.c(i, j, k).a()), season, bl);
                                if (n == null) continue;
                                chunkSection.setBiome(i, j, k, Holder.a((Object)((BiomeBase)NMSBiome_1_21_10.registrywritable.a(n.intValue()))));
                            }
                        }
                    }
                }
                try {
                    Field field = clientboundLevelChunkPacketData.getClass().getDeclaredField("d");
                    field.setAccessible(true);
                    field.set(clientboundLevelChunkPacketData, Packets_1_21_10.this.convertToBytes(chunkSectionArray));
    ...
    public ChunkSection[] getChunkSections(ClientboundLevelChunkPacketData clientboundLevelChunkPacketData, World world) {
        ...
        ChunkSection[] chunkSectionArray = new ChunkSection[((CraftWorld)world).getHandle().as()];
        DedicatedServer dedicatedServer = ((CraftServer)Bukkit.getServer()).getServer();
        IRegistryCustom.Dimension dimension = dedicatedServer.bg();
        PalettedContainerFactory palettedContainerFactory = PalettedContainerFactory.a((IRegistryCustom)dimension);
        for (int i = 0; i < chunkSectionArray.length; ++i) {
            ChunkSection chunkSection = new ChunkSection(palettedContainerFactory);
