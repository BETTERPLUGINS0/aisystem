...
public class CoinsEngineEconomyV1 implements EconomyProvider {
   ...
   public CoinsEngineEconomyV1(String currency) {
      if (!currency.isEmpty()) {
   ...
   private void registerCurrency() throws EconomyLoadException {
      ...
      if (this.currency != null && !this.currency.isEmpty()) {
         ...
         this.cur = (Currency)CoinsEngineAPI.getCurrencyManager().getCurrencies().stream().findFirst().orElse((Object)null);
         if (this.cur == null) {
   ...
   public double getBalance(OfflinePlayer p) {
      CoinsUser user = (CoinsUser)CoinsEngineAPI.getUserManager().getUserData(p.getUniqueId());
      return user.getCurrencyData(this.cur).getBalance();
   }
   ...
   public void depositBalance(OfflinePlayer p, double amount) {
      CoinsEngineAPI.getUserManager().getUserDataAsync(p.getUniqueId()).thenAccept((i) -> {
         if (i == null) {
            SendMessage.errorMessage("Failed to deposit balance: Cannot find player " + p.getName() + " in CoinsEngine");
         } else {
            i.getCurrencyData(this.cur).addBalance(amount);
            i.saveData(CoinsEngineAPI.PLUGIN);
         }
   ...
   private void formatSingular(String key) {
      ...
      String format = ChatUtil.formatColors(this.cur.getFormat().replace("%amount%", "%price%").replace("%currency_symbol%", singular));
      if (!singular.isEmpty()) {
   ...
   private void formatPlural(String key) {
      ...
      String format = ChatUtil.formatColors(this.cur.getFormat().replace("%amount%", "%price%").replace("%currency_symbol%", plural));
      if (!plural.isEmpty()) {
