...
public class BentoBoxAPI {
   public static boolean canPlayerStackBlock(@NotNull Player player, @NotNull Location location) {
      return BentoBox.getInstance().getIslandsManager().getIslandAt(location).isEmpty() ? true : (Boolean)BentoBox.getInstance().getIslandsManager().getIslandAt(location).map((island) -> {
         return island.isAllowed(User.getInstance(player.getUniqueId()), Flags.PLACE_BLOCKS);
   ...
   public static boolean canPlayerOpenMenu(@NotNull Player player, @NotNull Location location) {
      return BentoBox.getInstance().getIslandsManager().getIslandAt(location).isEmpty() ? true : (Boolean)BentoBox.getInstance().getIslandsManager().getIslandAt(location).map((island) -> {
         return island.isAllowed(User.getInstance(player.getUniqueId()), Flags.CONTAINER);
