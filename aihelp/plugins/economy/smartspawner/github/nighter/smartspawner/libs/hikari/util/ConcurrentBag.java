...
public class ConcurrentBag<T extends ConcurrentBag.IConcurrentBagEntry> implements AutoCloseable {
   ...
   public ConcurrentBag(ConcurrentBag.IBagStateListener listener) {
      ...
      this.useWeakThreadLocals = this.useWeakThreadLocals();
      this.handoffQueue = new SynchronousQueue(true);
      this.waiters = new AtomicInteger();
      this.sharedList = new CopyOnWriteArrayList();
      this.threadLocalList = ThreadLocal.withInitial(() -> {
         return (List)(this.useWeakThreadLocals ? new ArrayList(16) : new FastList(ConcurrentBag.IConcurrentBagEntry.class, 16));
      });
   }
   ...
   public T borrow(long timeout, TimeUnit timeUnit) throws InterruptedException {
      ...
      for(waiting = list.size() - 1; waiting >= 0; --waiting) {
         ...
         bagEntry = this.useWeakThreadLocals ? (ConcurrentBag.IConcurrentBagEntry)((WeakReference)entry).get() : (ConcurrentBag.IConcurrentBagEntry)entry;
   ...
   public void requite(T bagEntry) {
      ...
      for(int waiting = this.waiters.get(); waiting > 0; waiting = this.waiters.get()) {
         if (bagEntry.getState() != 0 || this.handoffQueue.offer(bagEntry)) {
            return;
         }

         if ((i & 255) != 255 && (waiting <= 1 || i % waiting != 0)) {
            Thread.yield();
         } else {
      ...
      List<Object> threadLocalEntries = (List)this.threadLocalList.get();
      if (threadLocalEntries.size() < 16) {
         threadLocalEntries.add(this.useWeakThreadLocals ? new WeakReference(bagEntry) : bagEntry);
      }
   ...
   public boolean remove(T bagEntry) {
      if (!bagEntry.compareAndSet(1, -1) && !bagEntry.compareAndSet(-2, -1) && !this.closed) {
         ...
         ((List)this.threadLocalList.get()).remove(bagEntry);
         return removed;
      }
   }
   ...
   public int getCount(int state) {
      ...
      while(var3.hasNext()) {
         T e = (ConcurrentBag.IConcurrentBagEntry)var3.next();
         if (e.getState() == state) {
            ++count;
         }
      }
   ...
   private boolean useWeakThreadLocals() {
      try {
         if (System.getProperty("github.nighter.smartspawner.libs.hikari.useWeakReferences") != null) {
            return Boolean.getBoolean("github.nighter.smartspawner.libs.hikari.useWeakReferences");
         } else {
