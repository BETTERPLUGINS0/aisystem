...
public class SignedSmallIntColumn extends ColumnDefinitionPacket implements ColumnDecoder {
   public SignedSmallIntColumn(ReadableByteBuf buf, int charset, long length, DataType dataType, byte decimals, int flags, int[] stringPos, String extTypeName, String extTypeFormat) {
   ...
   public int getPrecision() {
      return Math.min(5, (int)this.columnLength);
   }
   ...
   public String getColumnTypeName(Configuration conf) {
      return this.isSigned() ? "SMALLINT" : "SMALLINT UNSIGNED";
   }
   ...
   public byte decodeByteBinary(ReadableByteBuf buf, MutableInt length) throws SQLDataException {
      long result = (long)buf.readShort();
      if ((long)((byte)((int)result)) != result) {
         throw new SQLDataException("byte overflow");
      } else {
         return (byte)((int)result);
