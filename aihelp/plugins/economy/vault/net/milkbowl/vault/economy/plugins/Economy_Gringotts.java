...
public class Economy_Gringotts extends AbstractEconomy {
   ...
   public Economy_Gringotts(Plugin plugin) {
      ...
      Bukkit.getServer().getPluginManager().registerEvents(new Economy_Gringotts.EconomyServerListener(this), plugin);
      if (this.gringotts == null) {
         ...
         if (grngts != null && grngts.isEnabled()) {
            ...
            this.log.info(String.format("[Economy] %s hooked.", "Gringotts"));
         }
      }
   ...
   public EconomyResponse withdrawPlayer(String playerName, double amount) {
      if (amount < 0.0D) {
         ...
         if (accountHolder == null) {
            return new EconomyResponse(0.0D, 0.0D, EconomyResponse.ResponseType.FAILURE, playerName + " is not a valid account holder.");
         } else {
            Account account = this.gringotts.accounting.getAccount(accountHolder);
            return account.balance() >= amount && account.remove(amount) ? new EconomyResponse(amount, account.balance(), EconomyResponse.ResponseType.SUCCESS, (String)null) : new EconomyResponse(0.0D, account.balance(), EconomyResponse.ResponseType.FAILURE, "Insufficient funds");
         }
      }
   }
   ...
   public EconomyResponse depositPlayer(String playerName, double amount) {
      if (amount < 0.0D) {
         ...
         if (accountHolder == null) {
            ...
            Account account = this.gringotts.accounting.getAccount(accountHolder);
            return account.add(amount) ? new EconomyResponse(amount, account.balance(), EconomyResponse.ResponseType.SUCCESS, (String)null) : new EconomyResponse(0.0D, account.balance(), EconomyResponse.ResponseType.FAILURE, "Not enough capacity to store that amount!");
         }
      }
   }
   ...
   public class EconomyServerListener implements Listener {
      ...
      public void onPluginEnable(PluginEnableEvent event) {
         if (this.economy.gringotts == null) {
            ...
            if (grngts.getDescription().getName().equals("Gringotts")) {
               ...
               Economy_Gringotts.this.log.info(String.format("[Economy] %s hooked.", "Gringotts"));
            }
         }
      ...
      public void onPluginDisable(PluginDisableEvent event) {
         if (this.economy.gringotts != null && event.getPlugin().getDescription().getName().equals("Gringotts")) {
            ...
            Economy_Gringotts.this.log.info(String.format("[Economy] %s unhooked.", "Gringotts"));
         }
