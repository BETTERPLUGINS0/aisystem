...
public class ChickenShop extends BabyableShop<Chicken> {
   ...
   public ChickenShop(BaseEntityShopObjectCreationContext context, BaseEntityShopObjectType<ChickenShop> shopObjectType, AbstractShopkeeper shopkeeper, @Nullable ShopCreationData creationData) {
      super(context, shopObjectType, shopkeeper, creationData);
      PropertyValue var10001 = new PropertyValue(VARIANT);
      ChickenShop var10002 = (ChickenShop)Unsafe.initialized(this);
   ...
   private void applyVariant() {
      Chicken entity = (Chicken)this.getEntity();
      if (entity != null) {
   ...
   private ItemStack getVariantEditorItem() {
      ...
      Color color = (Color)VARIANT_EDITOR_ITEM_COLORS.getOrDefault(this.getVariant(), Color.WHITE);
      ItemUtils.setLeatherColor(iconItem, color);
      ItemUtils.setDisplayNameAndLore(iconItem, Messages.buttonChickenVariant, Messages.buttonChickenVariantLore);
      return iconItem;
   }
   ...
   private Button getVariantEditorButton() {
      return new ShopkeeperActionButton() {
         ...
         public ItemStack getIcon(EditorView editorView) {
            return ChickenShop.this.getVariantEditorItem();
         }

         protected boolean runAction(EditorView editorView, InventoryClickEvent clickEvent) {
            boolean backwards = clickEvent.isRightClick();
            ChickenShop.this.cycleVariant(backwards);
            return true;
         }
      };
   }
   ...
   static {
      ...
      VARIANT_EDITOR_ITEM_COLORS = Map.ofEntries(new Entry[]{Map.entry(Variant.TEMPERATE, Color.WHITE), Map.entry(Variant.WARM, Color.fromRGB(230, 178, 100)), Map.entry(Variant.COLD, Color.fromRGB(116, 106, 125))});
   }
}
