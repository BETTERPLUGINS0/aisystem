package com.volmit.iris.util.hunk.bits;

import com.volmit.iris.util.data.Varint;
import java.io.DataInputStream;
import java.io.DataOutputStream;
import java.util.concurrent.atomic.AtomicLongArray;
import java.util.function.IntConsumer;
import lombok.Generated;
import org.apache.commons.lang3.Validate;

public class DataBits {
   private static final int[] MAGIC = new int[]{-1, -1, 0, Integer.MIN_VALUE, 0, 0, 1431655765, 1431655765, 0, Integer.MIN_VALUE, 0, 1, 858993459, 858993459, 0, 715827882, 715827882, 0, 613566756, 613566756, 0, Integer.MIN_VALUE, 0, 2, 477218588, 477218588, 0, 429496729, 429496729, 0, 390451572, 390451572, 0, 357913941, 357913941, 0, 330382099, 330382099, 0, 306783378, 306783378, 0, 286331153, 286331153, 0, Integer.MIN_VALUE, 0, 3, 252645135, 252645135, 0, 238609294, 238609294, 0, 226050910, 226050910, 0, 214748364, 214748364, 0, 204522252, 204522252, 0, 195225786, 195225786, 0, 186737708, 186737708, 0, 178956970, 178956970, 0, 171798691, 171798691, 0, 165191049, 165191049, 0, 159072862, 159072862, 0, 153391689, 153391689, 0, 148102320, 148102320, 0, 143165576, 143165576, 0, 138547332, 138547332, 0, Integer.MIN_VALUE, 0, 4, 130150524, 130150524, 0, 126322567, 126322567, 0, 122713351, 122713351, 0, 119304647, 119304647, 0, 116080197, 116080197, 0, 113025455, 113025455, 0, 110127366, 110127366, 0, 107374182, 107374182, 0, 104755299, 104755299, 0, 102261126, 102261126, 0, 99882960, 99882960, 0, 97612893, 97612893, 0, 95443717, 95443717, 0, 93368854, 93368854, 0, 91382282, 91382282, 0, 89478485, 89478485, 0, 87652393, 87652393, 0, 85899345, 85899345, 0, 84215045, 84215045, 0, 82595524, 82595524, 0, 81037118, 81037118, 0, 79536431, 79536431, 0, 78090314, 78090314, 0, 76695844, 76695844, 0, 75350303, 75350303, 0, 74051160, 74051160, 0, 72796055, 72796055, 0, 71582788, 71582788, 0, 70409299, 70409299, 0, 69273666, 69273666, 0, 68174084, 68174084, 0, Integer.MIN_VALUE, 0, 5};
   private final AtomicLongArray data;
   private final int bits;
   private final long mask;
   private final int size;
   private final int valuesPerLong;
   private final int divideMul;
   private final int divideAdd;
   private final int divideShift;

   public DataBits(int bits, int length) {
      this(var1, var2, (AtomicLongArray)null);
   }

   public DataBits(int bits, int length, DataInputStream din) {
      this(var1, var2, longs(var3, dataLength(var1, var2)));
   }

   public DataBits(int bits, int length, AtomicLongArray data) {
      Validate.inclusiveBetween(1L, 32L, (long)var1);
      this.size = var2;
      this.bits = var1;
      this.mask = (1L << var1) - 1L;
      this.valuesPerLong = (char)(64 / var1);
      int var4 = 3 * (this.valuesPerLong - 1);
      this.divideMul = MAGIC[var4];
      this.divideAdd = MAGIC[var4 + 1];
      this.divideShift = MAGIC[var4 + 2];
      int var5 = (var2 + this.valuesPerLong - 1) / this.valuesPerLong;
      if (var3 != null) {
         if (var3.length() != var5) {
            throw new RuntimeException("NO! Trying to load " + var3.length() + " into actual size of " + var5 + " because length: " + var2 + " (bits: " + var1 + ")");
         }

         this.data = var3;
      } else {
         this.data = new AtomicLongArray(var5);
      }

   }

   private static int dataLength(int bits, int length) {
      return (var1 + (char)(64 / var0) - 1) / (char)(64 / var0);
   }

   private static AtomicLongArray longs(DataInputStream din, int longSize) {
      AtomicLongArray var2 = new AtomicLongArray(var1);

      for(int var3 = 0; var3 < var2.length(); ++var3) {
         var2.set(var3, Varint.readUnsignedVarLong(var0));
      }

      return var2;
   }

   public String toString() {
      int var10000 = this.size;
      return "DBits: " + var10000 + "/" + this.bits + "[" + this.data.length() + "]";
   }

   private int cellIndex(int var0) {
      long var2 = Integer.toUnsignedLong(this.divideMul);
      long var4 = Integer.toUnsignedLong(this.divideAdd);
      return (int)((long)var1 * var2 + var4 >> 32 >> this.divideShift);
   }

   public int getAndSet(int var0, int var1) {
      Validate.inclusiveBetween(0L, (long)(this.size - 1), (long)var1);
      Validate.inclusiveBetween(0L, this.mask, (long)var2);
      int var3 = this.cellIndex(var1);
      long var4 = this.data.get(var3);
      int var6 = (var1 - var3 * this.valuesPerLong) * this.bits;
      int var7 = (int)(var4 >> var6 & this.mask);
      this.data.set(var3, var4 & ~(this.mask << var6) | ((long)var2 & this.mask) << var6);
      return var7;
   }

   public void set(int var0, int var1) {
      Validate.inclusiveBetween(0L, (long)(this.size - 1), (long)var1);
      Validate.inclusiveBetween(0L, this.mask, (long)var2);
      int var3 = this.cellIndex(var1);
      long var4 = this.data.get(var3);
      int var6 = (var1 - var3 * this.valuesPerLong) * this.bits;
      this.data.set(var3, var4 & ~(this.mask << var6) | ((long)var2 & this.mask) << var6);
   }

   public int get(int var0) {
      Validate.inclusiveBetween(0L, (long)(this.size - 1), (long)var1);
      int var2 = this.cellIndex(var1);
      long var3 = this.data.get(var2);
      int var5 = (var1 - var2 * this.valuesPerLong) * this.bits;
      return (int)(var3 >> var5 & this.mask);
   }

   public AtomicLongArray getRaw() {
      return this.data;
   }

   public DataBits setBits(int newBits) {
      if (this.bits == var1) {
         return this;
      } else {
         DataBits var2 = new DataBits(var1, this.size);

         for(int var3 = 0; var3 < this.size; ++var3) {
            var2.set(var3, this.get(var3));
         }

         return var2;
      }
   }

   public void getAll(IntConsumer var0) {
      int var2 = 0;

      for(int var3 = 0; var3 < this.data.length(); ++var3) {
         long var4 = this.data.get(var3);

         for(int var6 = 0; var6 < this.valuesPerLong; ++var6) {
            var1.accept((int)(var4 & this.mask));
            var4 >>= this.bits;
            ++var2;
            if (var2 >= this.size) {
               return;
            }
         }
      }

   }

   public void write(DataOutputStream dos) {
      for(int var2 = 0; var2 < this.data.length(); ++var2) {
         Varint.writeUnsignedVarLong(this.data.get(var2), var1);
      }

   }

   @Generated
   public int getBits() {
      return this.bits;
   }

   @Generated
   public int getSize() {
      return this.size;
   }
}
