...
public class PopulatorData extends PopulatorDataAbstract implements IPopulatorDataBaseHeightAccess {
      ...
      if (entityTypesDict.isEmpty()) {
         ...
         for(int var8 = 0; var8 < var7; ++var8) {
            ...
            if (type != EntityType.ENDER_SIGNAL && type != EntityType.UNKNOWN) {
               try {
                  EntityTypes<?> et = (EntityTypes)EntityTypes.class.getDeclaredField(EntityTypeMapper.getObfsNameFromBukkitEntityType(type)).get((Object)null);
   ...
   public void addEntity(int rawX, int rawY, int rawZ, EntityType type) {
      if (Math.abs((rawX >> 4) - this.chunkX) <= 1 && Math.abs((rawZ >> 4) - this.chunkZ) <= 1) {
         try {
            EntityTypes<?> et = (EntityTypes)entityTypesDict.get(type);
            Entity e = et.a(this.rlwa.getMinecraftWorld());
            e.o((double)rawX + 0.5D, (double)rawY, (double)rawZ + 0.5D);
            if (e instanceof EntityInsentient) {
               ((EntityInsentient)e).a(this.rlwa, this.rlwa.d_(new BlockPosition((double)rawX + 0.5D, (double)rawY, (double)rawZ + 0.5D)), EnumMobSpawn.n, (GroupDataEntity)null, (NBTTagCompound)null);
            }

            this.rlwa.b(e);
         } catch (SecurityException | IllegalArgumentException var7) {
   ...
   public void lootTableChest(int x, int y, int z, @NotNull TerraLootTable table) {
      ...
      TileEntityLootable.a(this.rlwa, this.gen.getTerraformWorld().getHashedRand((long)x, y, z), pos, this.getLootTable(table));
   }
   ...
   public int getBaseHeight(int rawX, int rawZ) {
      return this.gen.a(rawX, rawZ, Type.a, this.rlwa);
   }
}
