...
public class MansionBaseTowerPiece extends MansionStandardTowerPiece {
   public MansionBaseTowerPiece(MansionJigsawBuilder builder, int widthX, int height, int widthZ, JigsawType type, BlockFace[] validDirs) {
   ...
   public void decorateAwkwardCorner(@NotNull Wall target, Random random, @NotNull BlockFace one, @NotNull BlockFace two) {
      ...
      (new SlabBuilder(Material.COBBLESTONE_SLAB)).setType(Type.TOP).apply(target.getRelative(one).getRelative(two).getUp(3));
      (new StairBuilder(Material.COBBLESTONE_STAIRS)).setFacing(one.getOppositeFace()).apply(target.getRelative(two).getUp(4)).setFacing(two.getOppositeFace()).apply(target.getRelative(one).getUp(4));
      target.getRelative(one).Pillar(4, new Material[]{Material.COBBLESTONE_WALL});
      target.getRelative(one).CorrectMultipleFacing(4);
      target.getRelative(two).Pillar(4, new Material[]{Material.COBBLESTONE_WALL});
