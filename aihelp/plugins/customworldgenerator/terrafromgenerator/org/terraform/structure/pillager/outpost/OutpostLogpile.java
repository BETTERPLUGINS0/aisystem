...
public class OutpostLogpile extends RoomPopulatorAbstract {
   ...
   public void populate(@NotNull PopulatorDataAbstract data, @NotNull CubeRoom room) {
      ...
      for(i = 0; i <= stackHeight; ++i) {
         for(width = 0; width < stackHeight - i; ++width) {
      ...
      for(i = -4; i <= 4; ++i) {
         for(width = -4; width <= 4; ++width) {
            core.getRelative(i, 0, width).get().lsetType(planksMat);
            core.getRelative(i, -1, width).downUntilSolid(new Random(), new Material[]{fenceMat});
         }
      }
