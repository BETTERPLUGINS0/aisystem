...
public class MansionGroundLevelForgePopulator extends MansionRoomPopulator {
   ...
   public void decorateRoom(@NotNull PopulatorDataAbstract data, @NotNull Random random) {
      ...
      try {
         ...
         if (randomFace == BlockFace.NORTH) {
            ...
            schema = TerraSchematic.load("mansion/mansion-forge", target);
            schema.setFace(randomFace);
            schema.parser = new MansionGroundLevelForgePopulator.MansionForgeSchematicParser(random, data);
            schema.apply();
   ...
   public void decorateWindow(Random rand, @NotNull Wall w) {
      (new StairBuilder(Material.POLISHED_ANDESITE_STAIRS)).setHalf(Half.TOP).setFacing(w.getDirection().getOppositeFace()).apply(w.getLeft()).apply(w.getRight()).setShape(Shape.OUTER_RIGHT).apply(w.getLeft(2)).setShape(Shape.OUTER_LEFT).apply(w.getRight(2));
      w.setType(new Material[]{Material.FLETCHING_TABLE, Material.SMITHING_TABLE});
   }

   public void decorateWall(@NotNull Random rand, @NotNull Wall w) {
      (new StairBuilder(Material.POLISHED_ANDESITE_STAIRS)).setHalf(Half.TOP).setFacing(BlockUtils.getLeft(w.getDirection())).apply(w.getLeft(3)).setFacing(BlockUtils.getRight(w.getDirection())).apply(w.getRight(3));
   ...
   private static class MansionForgeSchematicParser extends MansionRoomSchematicParser {
      ...
      public void applyData(@NotNull SimpleBlock block, @NotNull BlockData data) {
         ...
         if (data.getMaterial() == Material.POLISHED_DIORITE) {
            ArmorStandUtils.placeArmorStand(block.getUp(), BlockUtils.getDirectBlockFace(this.rand), this.rand);
         }
