...
public class MansionTowerStairwayPopulator extends MansionRoomPopulator {
   ...
   public void decorateRoom(@NotNull PopulatorDataAbstract data, @NotNull Random random) {
      ...
      for(int i = 1; i < height; ++i) {
         ...
         b.getRelative(face).getRelative(0, i + 1, 0).Pillar(3, new Material[]{Material.AIR});
      ...
      b = b.getUp(8);
      Iterator var11 = this.getRoom().getFourWalls(data, 2).entrySet().iterator();

      while(var11.hasNext()) {
         Entry<Wall, Integer> entry = (Entry)var11.next();
         Wall w = ((Wall)entry.getKey()).getAtY(b.getY());

         for(int i = 0; i < (Integer)entry.getValue(); ++i) {
            int pileHeight = GenUtils.randInt(random, 1, 2);
            w.Pillar(pileHeight, random, new Material[]{Material.CRAFTING_TABLE, Material.FLETCHING_TABLE, Material.CARTOGRAPHY_TABLE, Material.ANVIL, Material.NOTE_BLOCK, Material.SMITHING_TABLE});
