...
public class MansionLookoutTowerWallPiece extends MansionTowerWallPiece {
   public MansionLookoutTowerWallPiece(MansionJigsawBuilder builder, int widthX, int height, int widthZ, JigsawType type, BlockFace[] validDirs) {
         ...
         if (i != 0 && i != (Integer)entry.getValue() - 1) {
            if (i != 1 && i != (Integer)entry.getValue() - 2) {
               if (i != 2 && i != (Integer)entry.getValue() - 3) {
                  w.getUp().Pillar(5, new Material[]{Material.AIR});
                  (new StairBuilder(Material.STONE_BRICK_STAIRS)).setHalf(Half.TOP).setFacing(w.getDirection().getOppositeFace()).apply(w.getUp().getFront());
                  ...
                  (new StairBuilder(Material.DARK_OAK_STAIRS)).setHalf(Half.TOP).setFacing(BlockUtils.getLeft(w.getDirection())).apply(w.getUp(5).getLeft()).apply(w.getUp(4).getLeft(2)).setFacing(BlockUtils.getRight(w.getDirection())).apply(w.getUp(5).getRight()).apply(w.getUp(4).getRight(2));
                  w.getUp(6).getFront().setType(Material.STONE_BRICK_SLAB);
               } else {
                  w.getUp().Pillar(4, new Material[]{Material.AIR});
                  (new StairBuilder(Material.STONE_BRICK_STAIRS)).setHalf(Half.TOP).setFacing(w.getDirection().getOppositeFace()).apply(w.getUp().getFront());
                  (new DirectionalBuilder(Material.DARK_OAK_FENCE_GATE)).setFacing(w.getDirection()).apply(w.getUp(2).getFront());
                  w.getUp(2).getFront().CorrectMultipleFacing(1);
                  (new SlabBuilder(Material.STONE_BRICK_SLAB)).setType(Type.TOP).apply(w.getFront().getUp(5));
                  (new StairBuilder(Material.DARK_OAK_STAIRS)).setFacing(w.getDirection()).apply(w.getUp());
               }
            } else {
               w.getUp().Pillar(3, new Material[]{Material.AIR});
               (new StairBuilder(Material.STONE_BRICK_STAIRS)).setHalf(Half.TOP).setFacing(w.getDirection().getOppositeFace()).apply(w.getFront());
               (new OrientableBuilder(Material.STRIPPED_DARK_OAK_LOG)).setAxis(BlockUtils.getAxisFromBlockFace(w.getDirection())).apply(w.getUp()).apply(w.getUp().getFront());
               w.getUp(2).getFront().Pillar(3, new Material[]{Material.STONE_BRICK_WALL});
               w.getUp(2).getFront().CorrectMultipleFacing(3);
               w.getUp(5).getFront().setType(Material.STONE_BRICK_SLAB);
            }
         } else {
            w.getUp().Pillar(8, new Material[]{Material.DARK_OAK_LOG});
