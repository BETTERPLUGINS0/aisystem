...
public class MansionGrandStairwayPopulator extends MansionRoomPopulator {
   ...
   public void decorateWall(@NotNull Random rand, @NotNull Wall w) {
      w.getLeft(3).Pillar(7, new Material[]{Material.DARK_OAK_LOG});
      w.getUp(6).setType(Material.DARK_OAK_PLANKS);
      w.getRight(3).Pillar(7, new Material[]{Material.DARK_OAK_LOG});
      (new StairBuilder(Material.DARK_OAK_STAIRS)).setHalf(Half.TOP).setFacing(BlockUtils.getLeft(w.getDirection())).apply(w.getUp(5).getLeft(2)).apply(w.getUp(6).getLeft(2)).apply(w.getUp(6).getLeft()).setFacing(BlockUtils.getRight(w.getDirection())).apply(w.getUp(5).getRight(2)).apply(w.getUp(6).getRight(2)).apply(w.getUp(6).getRight());
      int choice = rand.nextInt(2);
      if (choice == 0) {
         w.getRear().getUp().Pillar(5, new Material[]{Material.DARK_OAK_LOG});
         (new OrientableBuilder(Material.DARK_OAK_LOG)).setAxis(BlockUtils.getAxisFromBlockFace(w.getDirection())).apply(w.getRear().getUp(3)).apply(w.getRear().getUp(2).getLeft()).apply(w.getRear().getUp(2).getRight()).apply(w.getRear().getUp(4).getLeft()).apply(w.getRear().getUp(4).getRight()).setAxis(BlockUtils.getAxisFromBlockFace(BlockUtils.getLeft(w.getDirection()))).apply(w.getRear().getUp(3).getLeft()).apply(w.getRear().getUp(3).getLeft(2)).apply(w.getRear().getUp(3).getRight()).apply(w.getRear().getUp(3).getRight(2));
      } else {
   ...
   public void decorateWindow(Random rand, @NotNull Wall w) {
      ...
      for(int i = 0; i < (Integer)entry.getValue(); ++i) {
         if (w.getFront().getUp(6).getType() == Material.DARK_OAK_SLAB) {
            w.getRear().Pillar(7, new Material[]{Material.DARK_OAK_PLANKS});
            w.Pillar(7, new Material[]{Material.DARK_OAK_LOG});
            (new StairBuilder(Material.DARK_OAK_STAIRS)).setFacing(w.getDirection().getOppositeFace()).apply(w.getFront());
         }
