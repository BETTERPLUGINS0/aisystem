...
public class PlainsVillageTempleEntrancePiece extends JigsawStructurePiece {
   ...
   public void build(@NotNull PopulatorDataAbstract data, @NotNull Random rand) {
      ...
      core = core.getRear(2);
      BlockUtils.placeDoor(data, this.plainsVillagePopulator.woodDoor, core.getX(), core.getY(), core.getZ(), core.getDirection());
      ...
      (new StairBuilder(Material.STONE_BRICK_STAIRS)).setHalf(Half.TOP).setFacing(core.getDirection().getOppositeFace()).apply(core.getFront().getUp(2));
      core.getFront().getUp(3).setType(Material.CHISELED_STONE_BRICKS);
      Wall doorAdj = core.getFront().getRight();
      (new StairBuilder(Material.STONE_BRICK_STAIRS)).setFacing(doorAdj.getDirection().getOppositeFace()).apply(doorAdj.getUp());
