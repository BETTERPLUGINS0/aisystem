         ...
         for(int i = 0; i < (Integer)entry.getValue(); ++i) {
            if (i != 0 && i != 4) {
               if (i != 1 && i != 3) {
                  ...
                  (new SlabBuilder(new Material[]{Material.STONE_BRICK_SLAB, Material.MOSSY_STONE_BRICK_SLAB})).setType(Type.TOP).apply(w.getUp(2));
                  w.getUp(3).LPillar(25, random, new Material[]{Material.BOOKSHELF});
               } else {
                  (new StairBuilder(this.plainsVillagePopulator.woodStairs)).setFacing(w.getDirection().getOppositeFace()).apply(w);
                  (new StairBuilder(new Material[]{Material.STONE_BRICK_STAIRS, Material.MOSSY_STONE_BRICK_STAIRS})).setFacing(w.getDirection().getOppositeFace()).setHalf(Half.TOP).apply(w.getUp(2));
                  w.getUp(3).LPillar(25, random, new Material[]{Material.BOOKSHELF});
               }
            } else {
               w.LPillar(25, random, new Material[]{Material.BOOKSHELF});
               if (GenUtils.chance(random, 1, 10)) {
                  Ladder ladder = (Ladder)Bukkit.createBlockData(Material.LADDER);
                  ladder.setFacing(w.getDirection());

                  for(int h = 0; h < 25 && !w.getFront().getRelative(0, h, 0).isSolid(); ++h) {
