...
public class NMSInjector extends NMSInjectorAbstract {
   ...
   public boolean attemptInject(@NotNull World world) {
      try {
         ...
         ChunkGenerator delegate = ws.m().g();
         ...
         PlayerChunkMap pcm = ws.m().a;
         Field wgc = pcm.getClass().getDeclaredField("R");
         wgc.setAccessible(true);
         WorldGenContext worldGenContext = (WorldGenContext)wgc.get(pcm);
         wgc.set(pcm, new WorldGenContext(worldGenContext.a(), bpg, worldGenContext.c(), worldGenContext.d(), worldGenContext.e(), worldGenContext.f()));
         TerraformGeneratorPlugin.logger.info("Post injection: getChunkSource().getChunkGenerator() is of type " + ws.m().g().getClass().getSimpleName());
         return true;
   ...
   public PopulatorDataICAAbstract getICAData(PopulatorDataAbstract data) {
      if (data instanceof PopulatorDataSpigotAPI) {
         ...
         TerraformWorld tw = TerraformWorld.get(ws.getWorld().getName(), ws.E());
