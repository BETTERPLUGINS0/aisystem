...
public class PopulatorData extends PopulatorDataAbstract implements IPopulatorDataBaseHeightAccess, IPopulatorDataBeehiveEditor {
   private static final HashMap<EntityType, EntityTypes<?>> EntityTypeDict = new HashMap();
   ...
   public void setSpawner(int rawX, int rawY, int rawZ, EntityType type) {
      if (TConfig.c.FEATURE_SPAWNERS_ENABLED) {
         ...
         Optional<TileEntityMobSpawner> opt = this.rlwa.a(pos, TileEntityTypes.j);
         if (opt.isPresent()) {
            TileEntityMobSpawner tileentity = (TileEntityMobSpawner)opt.get();

            try {
               EntityTypes<?> nmsEntity = (EntityTypes)EntityTypeDict.get(type);
               if (nmsEntity == null) {
   ...
   public void lootTableChest(int x, int y, int z, TerraLootTable table) {
      ...
      if (te instanceof TileEntityLootable) {
         ...
         bbe.a((ResourceKey)LootTableTranslator.translationMap.get(table), this.gen.getTerraformWorld().getHashedRand((long)x, y, z).nextLong());
      }
   ...
   public void setBeehiveWithBee(int rawX, int rawY, int rawZ) {
      ...
      try {
         ...
         tileentity.a(c.a(GenUtils.RANDOMIZER.nextInt(599)));
