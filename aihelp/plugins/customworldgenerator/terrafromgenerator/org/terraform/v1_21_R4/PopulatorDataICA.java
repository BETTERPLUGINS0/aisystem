...
public class PopulatorDataICA extends PopulatorDataICABiomeWriterAbstract {
   ...
   public void lootTableChest(int x, int y, int z, TerraLootTable table) {
      ...
      if (te instanceof TileEntityLootable) {
         ((TileEntityLootable)te).a((ResourceKey)LootTableTranslator.translationMap.get(table));
      } else if (te instanceof BrushableBlockEntity) {
         ((BrushableBlockEntity)te).a((ResourceKey)LootTableTranslator.translationMap.get(table), this.tw.getHashedRand((long)x, y, z).nextLong());
      }
   ...
   public void registerNaturalSpawns(@NotNull NaturalSpawnType type, int x0, int y0, int z0, int x1, int y1, int z1) {
      ...
      IRegistry<Structure> featureRegistry = (IRegistry)MinecraftServer.getServer().ba().a(Registries.be).orElseThrow();
      Structure structureFeature = (Structure)((c)featureRegistry.a(structureKey).get()).a();
      ...
      try {
         ...
         final StructurePiece customBoundPiece = (StructurePiece)oceanMonumentPiecesHClass.getConstructor(RandomSource.class, Integer.TYPE, Integer.TYPE, enumDirectionClass).newInstance(RandomSource.a(), x0, z0, enumDirectionA.get((Object)null));
         ...
         Field i = StructureStart.class.getDeclaredField("h");
      ...
      } catch (InstantiationException | InvocationTargetException | ClassNotFoundException | NoSuchFieldException | IllegalArgumentException | IllegalAccessException | NoSuchMethodException var18) {
   ...
   public void spawnMinecartWithChest(int x, int y, int z, TerraLootTable table, @NotNull Random random) {
      EntityMinecartChest entityminecartchest = (EntityMinecartChest)EntityTypes.z.a(this.ws.getMinecraftWorld(), EntitySpawnReason.b);
      if (entityminecartchest != null) {
         entityminecartchest.a_((double)((float)x + 0.5F), (double)((float)y + 0.5F), (double)((float)z + 0.5F));
         entityminecartchest.a((ResourceKey)LootTableTranslator.translationMap.get(table), random.nextLong());
