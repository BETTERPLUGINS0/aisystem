...
public class NMSInjectListener implements Listener {
   ...
   public void onWorldInit(WorldInitEvent event) {
      if (!INJECTED.contains(event.getWorld())) {
         ChunkGenerator var3 = event.getWorld().getGenerator();
         if (var3 instanceof BukkitChunkGeneratorWrapper) {
            BukkitChunkGeneratorWrapper bukkitChunkGeneratorWrapper = (BukkitChunkGeneratorWrapper)var3;
            INJECT_LOCK.lock();
            ...
            net.minecraft.world.level.chunk.ChunkGenerator vanilla = serverWorld.getChunkSource().getGenerator();
            ...
            Reflection.CHUNKMAP.setWorldGenContext(chunkMap, new WorldGenContext(worldGenContext.level(), new NMSChunkGeneratorDelegate(vanilla, pack, provider, craftWorld.getSeed()), worldGenContext.structureManager(), worldGenContext.lightEngine(), worldGenContext.mainThreadExecutor(), worldGenContext.unsavedListener()));
