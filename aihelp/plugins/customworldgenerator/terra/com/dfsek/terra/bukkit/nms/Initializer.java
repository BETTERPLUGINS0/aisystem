...
public interface Initializer {
   ...
   private static PlatformImpl constructPlatform(TerraBukkitPlugin plugin) {
      try {
         ...
         try {
            return (PlatformImpl)platformClass.getConstructor(TerraBukkitPlugin.class).newInstance(plugin);
         } catch (ReflectiveOperationException var4) {
            throw new RuntimeException("Error initializing NMS bindings. Report this to Terra.", var4);
         }
