...
public class SellMax extends SubCmd {
   ...
   public void onCommand(@NotNull CommandSender sender, @NotNull String alias, String[] args) {
      try {
         ...
         if (!economy.has(target, price / (double)amount)) {
            if (!silent) {
               this.sendLanguageString("not-enough-money", (String)null, target, new String[]{"%id%", args[1].toLowerCase(), "%nick%", ItemEdit.get().getServerStorage().getNick(args[1]), "%amount%", String.valueOf(amount), "%price%", economy.format(price)});
            }
         ...
         if (bal < price) {
            ...
            price = pricePerItem * (double)amount;
         }
         ...
         if (given == 0) {
            if (!silent) {
               this.sendLanguageString("not-enough-space", (String)null, target, new String[]{"%id%", args[1].toLowerCase(), "%nick%", ItemEdit.get().getServerStorage().getNick(args[1])});
            }
         ...
         if (given < amount) {
            if (!economy.depositPlayer(target, pricePerItem * (double)(amount - given)).transactionSuccess()) {
               if (!silent) {
                  this.sendLanguageString("transaction-failed", (String)null, target, new String[0]);
               }

               Util.logToFile("[transaction failed] player '" + target.getName() + "' (" + target.getUniqueId() + ") lost " + economy.format(pricePerItem * (double)(amount - given)) + " money, is your Economy provider stable?");
               return;
            }

            amount = given;
            price = pricePerItem * (double)given;
         }
