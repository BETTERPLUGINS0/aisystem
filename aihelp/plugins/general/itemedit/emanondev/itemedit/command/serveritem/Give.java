   ...
   public void onCommand(@NotNull CommandSender sender, @NotNull String alias, String[] args) {
      try {
         ...
         if (given > 0 && ItemEdit.get().getConfig().loadBoolean("log.action.give", true)) {
            String msg = UtilsString.fix(this.getConfigString("log", new String[0]), target, true, "%id%", args[1].toLowerCase(), "%nick%", ItemEdit.get().getServerStorage().getNick(args[1]), "%amount%", String.valueOf(given), "%player_name%", target.getName());
