...
public class VaultSupport {
   ...
   public boolean setupEconomy() {
      ...
      if (serviceProvider != null) {
         this.vault = (Economy)serviceProvider.getProvider();
      }
   ...
   public boolean removeMoney(@NotNull Player player, long amount) {
      EconomyResponse result = this.vault.withdrawPlayer(player, (double)amount);
   ...
   public boolean addMoney(Player player, long amount) {
      EconomyResponse result = this.vault.depositPlayer(player, (double)amount);
      return result.transactionSuccess();
   }
