...
public class CoinsEngineEconomyV5 implements EconomyProvider {
   private ExcellentEconomyAPI api;
   private String currency;
   private ExcellentCurrency cur;
   ...
   public CoinsEngineEconomyV5(String currency) {
      this.currency = currency;
   }

   public void setup(EconomyShopGUI plugin) throws EconomyLoadException {
      if (!plugin.getServer().getPluginManager().isPluginEnabled("CoinsEngine") && !plugin.getServer().getPluginManager().isPluginEnabled("ExcellentEconomy")) {
         throw new EconomyLoadException("CoinsEngine not enabled yet");
      } else {
         ...
         if (provider != null) {
            this.api = (ExcellentEconomyAPI)provider.getProvider();
         }
   ...
   private void registerCurrency() throws EconomyLoadException {
      ...
      if (this.currency != null && !this.currency.isEmpty()) {
         this.cur = this.api.getCurrency(this.currency);
         if (this.cur == null) {
         ...
         this.cur = (ExcellentCurrency)this.api.getCurrencies().stream().findFirst().orElse((Object)null);
         if (this.cur == null) {
   ...
   public double getBalance(OfflinePlayer p) {
      return this.api.getBalance((Player)p, this.cur);
   }

   public void depositBalance(OfflinePlayer p, double amount) {
      this.api.deposit((Player)p, this.cur, amount);
   }

   public void withdrawBalance(OfflinePlayer p, double amount) {
      this.api.withdraw((Player)p, this.cur, amount);
   }
   ...
   private void formatSingular(String key) {
      ...
      if (!singular.isEmpty()) {
         if (this.isSymbol(singular)) {
            this.singular = format.isEmpty() ? Lang.SYMBOL_PRICING_FORMAT.get().replace("%symbol%", singular).getLegacy() : format;
         } else {
            this.singular = format.isEmpty() ? Lang.NAMED_PRICING_FORMAT.get().replace("%currency-format%", singular).getLegacy() : format;
         }
      } else {
         this.singular = format.isEmpty() ? Lang.SYMBOL_PRICING_FORMAT.get().replace("%symbol%", Lang.CURRENCYSYMBOL.get()).getLegacy() : format;
      }
   ...
   private void formatPlural(String key) {
      ...
      if (!plural.isEmpty()) {
         if (this.isSymbol(plural)) {
            this.plural = format.isEmpty() ? Lang.SYMBOL_PRICING_FORMAT.get().replace("%symbol%", plural).getLegacy() : format;
         } else {
            this.plural = format.isEmpty() ? Lang.NAMED_PRICING_FORMAT.get().replace("%currency-format%", plural).getLegacy() : format;
         }
      } else {
         this.plural = format.isEmpty() ? Lang.SYMBOL_PRICING_FORMAT.get().replace("%symbol%", Lang.CURRENCYSYMBOL.get()).getLegacy() : format;
      }
