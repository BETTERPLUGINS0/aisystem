...
public class PlayerProfile implements NavItem {
   private ItemStack navBarItem;
   private String action;
   private boolean disabled;
   private boolean onlyGeyser;
   private PlayerProfile.Paginated paginatedItem;
   ...
   public PlayerProfile(ConfigurationSection config, ItemStack fillItem) throws ItemLoadException {
      ...
      if (item.getType().equals(Material.AIR)) {
         ...
         throw new ItemLoadException("Item material needs to be of type PLAYER_HEAD when using option skullowner: %player_name%");
      } else {
         ...
         if (config.getBoolean("paginated.enabled")) {
            ...
            if (section != null && !section.getKeys(false).isEmpty()) {
               this.paginatedItem = new PlayerProfile.Paginated(section);
            } else {
               this.paginatedItem = new PlayerProfile.Paginated(fillItem);
            }
         }
   ...
   public ItemStack getItem(Player p, EcoType type, int page, int allpages) {
      if (this.paginatedItem == null || allpages != 1 && (!this.action.equals("PAGE_BACK") || page != 1) && (!this.action.equals("PAGE_NEXT") || page != allpages)) {
         ItemStack item;
         if (UserManager.getUser(p).isBedrock()) {
            item = EconomyShopGUI.getInstance().navBar.getCustomPlayerProfile();
            if (item == null) {
               item = new ItemStack(this.navBarItem);
               if (!item.hasItemMeta()) {
                  return item;
               }

               SkullUtil.setSkullTexture(item, (SkullMeta)null, p.getName(), true);
            }
         } else {
            item = new ItemStack(this.navBarItem);
            if (!item.hasItemMeta()) {
   ...
   public class Paginated {
      ...
      public Paginated(ConfigurationSection param2) throws ItemLoadException {
         ...
         if (item.getType().equals(Material.AIR)) {
            ...
            if (name != null) {
               Translatable displayname = PlayerProfile.this.replaceTranslations(name);
               EconomyShopGUI.getInstance().getMetaUtils().setDisplayName(meta, displayname);
            }
            ...
            if (!l.isEmpty()) {
               List<Translatable> lore = (List)l.stream().map((x$0) -> {
                  return PlayerProfile.this.replaceTranslations(x$0);
               }).collect(Collectors.toList());
               EconomyShopGUI.getInstance().getMetaUtils().setLore(meta, lore);
            }

            item.setItemMeta(meta);
            this.placeholders = PlayerProfile.this.getPlaceholders(item);
            this.action = PlayerProfile.this.getAction(config, (ItemStack)null);
            this.orElse = item;
         }
      }
      ...
      public ItemStack getItem(Player p, EcoType type, int page, int allpages) {
         if (this.staticItem) {
            ...
            if (!item.hasItemMeta()) {
               ...
               ItemMeta meta = item.getItemMeta();
               if (this.placeholders.contains(-1)) {
                  EconomyShopGUI.getInstance().getMetaUtils().setRawDisplayName(meta, PlayerProfile.this.replacePlaceHolders(p, EconomyShopGUI.getInstance().getMetaUtils().getRawDisplayName(meta), type, page, allpages));
               }
               ...
               if (meta.hasLore()) {
                  ...
                  while(var8.hasNext()) {
                     Integer i = (Integer)var8.next();
                     if (i != -1) {
                        lore.set(i, PlayerProfile.this.replacePlaceHolders(p, (String)lore.get(i), type, page, allpages));
                     }
                  }
