...
public class Commands {
   ...
   public void load() {
      this.commands.clear();
      RTPCommandType[] var1 = RTPCommandType.values();
      int var2 = var1.length;

      for(int var3 = 0; var3 < var2; ++var3) {
         RTPCommandType cmd = var1[var3];
   ...
   public void registerCommand(RTPCommand cmd, boolean forced) {
      if (!cmd.isDebugOnly() || this.pl.getSettings().isDebug() || forced) {
   ...
   public void commandExecuted(CommandSender sendi, String label, String[] args) {
      if (PermissionNode.USE.check(sendi)) {
         if (args != null && args.length > 0) {
            ...
            while(var7.hasNext()) {
               RTPCommand cmd = (RTPCommand)var7.next();
               if (cmd.getName().equalsIgnoreCase(args[0])) {
                  if (cmd.permission().check(sendi)) {
                     ...
                     MessagesCore.NOPERMISSION.send(sendi, (Object)cmd);
                  }
            ...
            MessagesCore.INVALID.send(sendi, (Object)label);
         } else {
         ...
         MessagesCore.NOPERMISSION.send(sendi, (Object)PermissionNode.USE);
      }
   ...
   public List<String> onTabComplete(CommandSender sendi, String[] args) {
      ...
      if (args.length == 1) {
         ...
         while(var4.hasNext()) {
            cmd = (RTPCommand)var4.next();
            if (cmd.getName().toLowerCase().startsWith(args[0].toLowerCase()) && cmd.permission().check(sendi)) {
         ...
         while(var4.hasNext()) {
            cmd = (RTPCommand)var4.next();
            if (cmd.getName().equalsIgnoreCase(args[0]) && cmd.permission().check(sendi)) {
