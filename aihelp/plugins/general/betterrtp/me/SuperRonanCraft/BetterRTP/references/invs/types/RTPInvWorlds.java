...
public class RTPInvWorlds extends RTPInventory {
   ...
   public void clickEvent(InventoryClickEvent e) {
      ...
      int slot = e.getSlot();
      RTP_INV_ITEMS[] var4 = RTP_INV_ITEMS.values();
      int var5 = var4.length;

      for(int var6 = 0; var6 < var5; ++var6) {
         RTP_INV_ITEMS type = var4[var6];
         if (type.slot != -1) {
            switch(type) {
            case BACK:
      ...
      for(Iterator var9 = Bukkit.getWorlds().iterator(); var9.hasNext(); ++_index) {
         World world = (World)var9.next();
         if (_index == slot) {
            BetterRTP.getInstance().getPInfo().setInvWorld(p, world);
            BetterRTP.getInstance().getInvs().getInv((RTP_INV_SETTINGS)BetterRTP.getInstance().getPInfo().getInvNextInv().get(p)).show(p);
         }
      }
