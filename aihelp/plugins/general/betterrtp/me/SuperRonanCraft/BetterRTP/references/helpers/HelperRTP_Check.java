...
public class HelperRTP_Check {
   public static RTP_ERROR_REQUEST_REASON canRTP(Player player, CommandSender sendi, WorldPlayer pWorld, RTP_PlayerInfo rtpInfo) {
      ...
      } else if (sendi == player && !PermissionCheck.getAWorld(sendi, pWorld.getWorld().getName())) {
         return RTP_ERROR_REQUEST_REASON.NO_PERMISSION;
      } else if (getPl().getRTP().getDisabledWorlds().contains(pWorld.getWorld().getName())) {
         return RTP_ERROR_REQUEST_REASON.WORLD_DISABLED;
      } else if (rtpInfo.isCheckCooldown() && isCoolingDown(player, pWorld)) {
         return RTP_ERROR_REQUEST_REASON.COOLDOWN;
      } else if (rtpInfo.isTakeMoney() && !getPl().getEco().hasBalance(pWorld)) {
         return RTP_ERROR_REQUEST_REASON.PRICE_ECONOMY;
      } else {
         return rtpInfo.isTakeHunger() && !getPl().getEco().hasHunger(pWorld) ? RTP_ERROR_REQUEST_REASON.PRICE_HUNGER : null;
      }
   }

   private static boolean isRTPing(Player player) {
      return (Boolean)getPl().getPInfo().getRtping().getOrDefault(player, false);
   }
   ...
   public static long getCooldown(Player player, WorldPlayer pWorld) {
      ...
      if (cooldownHandler.isLoaded() && cooldownHandler.loadedPlayer(player)) {
         CooldownData cooldownData = getPl().getCooldowns().get(player, pWorld.getWorld());
         if (cooldownData != null) {
            if (cooldownData.getTime() == 0L) {
               return 0L;
            } else {
               return isLocked(player) ? -1L : cooldownHandler.timeLeft(player, cooldownData, pWorld);
            }
         } else {
            return 0L;
         }
   ...
   public static boolean applyCooldown(Player player) {
      return getPl().getCooldowns().isEnabled() && !PermissionNode.BYPASS_COOLDOWN.check(player);
   }

   public static boolean applyDelay(Player player) {
      return getPl().getSettings().isDelayEnabled() && getPl().getSettings().getDelayTime() > 0 && !PermissionNode.BYPASS_DELAY.check(player);
   }
