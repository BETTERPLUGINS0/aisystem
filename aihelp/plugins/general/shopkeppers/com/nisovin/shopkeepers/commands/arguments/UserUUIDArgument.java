...
public class UserUUIDArgument extends ObjectUUIDArgument {
   ...
   public UserUUIDArgument(String name, ArgumentFilter<? super UUID> filter, int minimumCompletionInput) {
   ...
   public static Iterable<? extends UUID> getDefaultCompletionSuggestions(CommandInput input, CommandContextView context, int minimumCompletionInput, String uuidPrefix, ArgumentFilter<? super User> userFilter) {
      if (uuidPrefix.length() < minimumCompletionInput) {
         ...
         }).map(User::getUniqueId).filter((uuid) -> {
