...
public class MimeBodyPart extends BodyPart implements MimePart {
   ...
   static void setFileName(MimePart part, String name) throws MessagingException {
      if (encodeFileName && name != null) {
         try {
            ...
            throw new MessagingException("Can't encode filename", var8);
         }
      }
   ...
   static String getEncoding(MimePart part) throws MessagingException {
      ...
      if (s == null) {
         ...
         if (s.length() == 0) {
            return null;
         } else if (!s.equalsIgnoreCase("7bit") && !s.equalsIgnoreCase("8bit") && !s.equalsIgnoreCase("quoted-printable") && !s.equalsIgnoreCase("binary") && !s.equalsIgnoreCase("base64")) {
            HeaderTokenizer h = new HeaderTokenizer(s, "()<>@,;:\\\"\t []/?=");
      ...
      if (dh != null) {
         try {
            ...
            if (dh instanceof MimeBodyPart.MimePartDataHandler) {
               ...
               filename = mpart.getEncoding();
               if (filename != null) {
                  setEncoding(part, filename);
                  return;
               }
            }
            ...
            throw new MessagingException("IOException updating headers", var10);
         }
      }
   }
   ...
   static void writeTo(MimePart part, OutputStream os, String[] ignoreList) throws IOException, MessagingException {
      ...
      try {
         DataHandler dh = part.getDataHandler();
         if (dh instanceof MimeBodyPart.MimePartDataHandler) {
            MimeBodyPart.MimePartDataHandler mpdh = (MimeBodyPart.MimePartDataHandler)dh;
