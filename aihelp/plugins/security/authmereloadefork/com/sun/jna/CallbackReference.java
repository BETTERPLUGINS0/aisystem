...
public class CallbackReference extends WeakReference<Callback> {
   ...
   private static Callback getCallback(Class<?> type, Pointer p, boolean direct) {
      if (p == null) {
         ...
         synchronized(pointerCallbackMap) {
            Reference<Callback>[] array = (Reference[])pointerCallbackMap.get(p);
   ...
   private static Callback getTypeAssignableCallback(Class<?> type, Reference<Callback>[] array) {
      if (array != null) {
         for(int i = 0; i < array.length; ++i) {
            Callback cb = (Callback)array[i].get();
   ...
   private CallbackReference(Callback callback, int callingConvention, boolean direct) {
      ...
      if (direct) {
         ...
         flags = DLL_CALLBACK_CLASS != null && DLL_CALLBACK_CLASS.isInstance(callback) ? 2 : 0;
      ...
      if (cb == null) {
         ...
         synchronized(pointerCallbackMap) {
            ...
            if (cbref == null) {
               ...
               map.put(cb, cbref);
               pointerCallbackMap.put(cbref.getTrampoline(), addCallbackToArray(cb, (Reference[])null));
   ...
   static {
      ...
      if (Platform.isWindows()) {
         try {
            ...
            throw new Error("Error loading DLLCallback class", var1);
         }
      } else {
