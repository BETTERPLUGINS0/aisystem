...
class ELFAnalyser {
   ...
   private void parseEabiAapcsVfp(ByteBuffer headerData, RandomAccessFile raf) throws IOException {
      ELFAnalyser.ELFSectionHeaders sectionHeaders = new ELFAnalyser.ELFSectionHeaders(this._64Bit, this.bigEndian, headerData, raf);
      ...
      while(true) {
         while(true) {
            Map fileAttributes;
            do {
               ELFAnalyser.ELFSectionHeaderEntry eshe;
               do {
                  if (!var4.hasNext()) {
                     return;
                  }

                  eshe = (ELFAnalyser.ELFSectionHeaderEntry)var4.next();
               ...
               Map<Integer, Map<ELFAnalyser.ArmAeabiAttributesTag, Object>> armAttributes = parseArmAttributes(armAttributesBuffer);
   ...
   private static Map<Integer, Map<ELFAnalyser.ArmAeabiAttributesTag, Object>> parseArmAttributes(ByteBuffer bb) {
      byte format = bb.get();
      if (format != 65) {
   ...
   private static Map<Integer, Map<ELFAnalyser.ArmAeabiAttributesTag, Object>> parseAEABI(ByteBuffer buffer) {
      HashMap data;
      int pos;
      int length;
   ...
   private static Map<ELFAnalyser.ArmAeabiAttributesTag, Object> parseFileAttribute(ByteBuffer bb) {
      HashMap result = new HashMap();

      while(bb.position() < bb.limit()) {
         int tagValue = readULEB128(bb).intValue();
         ELFAnalyser.ArmAeabiAttributesTag tag = ELFAnalyser.ArmAeabiAttributesTag.getByValue(tagValue);
   ...
   static class ArmAeabiAttributesTag {
      ...
      private final ELFAnalyser.ArmAeabiAttributesTag.ParameterType parameterType;
      private static final List<ELFAnalyser.ArmAeabiAttributesTag> tags = new LinkedList();
      private static final Map<Integer, ELFAnalyser.ArmAeabiAttributesTag> valueMap = new HashMap();
      private static final Map<String, ELFAnalyser.ArmAeabiAttributesTag> nameMap = new HashMap();
      ...
      public ArmAeabiAttributesTag(int value, String name, ELFAnalyser.ArmAeabiAttributesTag.ParameterType parameterType) {
         this.value = value;
         this.name = name;
      ...
      public boolean equals(Object obj) {
         if (this == obj) {
            ...
            ELFAnalyser.ArmAeabiAttributesTag other = (ELFAnalyser.ArmAeabiAttributesTag)obj;
            return this.value == other.value;
         }
      }

      private static ELFAnalyser.ArmAeabiAttributesTag addTag(int value, String name, ELFAnalyser.ArmAeabiAttributesTag.ParameterType type) {
         ELFAnalyser.ArmAeabiAttributesTag tag = new ELFAnalyser.ArmAeabiAttributesTag(value, name, type);
      ...
      public static List<ELFAnalyser.ArmAeabiAttributesTag> getTags() {
         return Collections.unmodifiableList(tags);
      }

      public static ELFAnalyser.ArmAeabiAttributesTag getByName(String name) {
         return (ELFAnalyser.ArmAeabiAttributesTag)nameMap.get(name);
      }

      public static ELFAnalyser.ArmAeabiAttributesTag getByValue(int value) {
         if (valueMap.containsKey(value)) {
            return (ELFAnalyser.ArmAeabiAttributesTag)valueMap.get(value);
         } else {
            ELFAnalyser.ArmAeabiAttributesTag pseudoTag = new ELFAnalyser.ArmAeabiAttributesTag(value, "Unknown " + value, getParameterType(value));
            return pseudoTag;
         }
      }

      private static ELFAnalyser.ArmAeabiAttributesTag.ParameterType getParameterType(int value) {
         ELFAnalyser.ArmAeabiAttributesTag tag = getByValue(value);
         if (tag == null) {
   ...
   static class ELFSectionHeaders {
      private final List<ELFAnalyser.ELFSectionHeaderEntry> entries = new ArrayList();
      ...
      public ELFSectionHeaders(boolean _64bit, boolean bigEndian, ByteBuffer headerData, RandomAccessFile raf) throws IOException {
         ...
         for(int i = 0; i < shnum; ++i) {
            ...
            this.entries.add(new ELFAnalyser.ELFSectionHeaderEntry(_64bit, stringBuffer));
         }

         ELFAnalyser.ELFSectionHeaderEntry stringTable = (ELFAnalyser.ELFSectionHeaderEntry)this.entries.get(shstrndx);
         ...
         for(Iterator var15 = this.entries.iterator(); var15.hasNext(); eshe.setName(baos.toString("ASCII"))) {
            eshe = (ELFAnalyser.ELFSectionHeaderEntry)var15.next();
            baos.reset();
