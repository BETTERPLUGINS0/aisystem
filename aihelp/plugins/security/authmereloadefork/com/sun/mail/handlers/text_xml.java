...
public class text_xml extends text_plain {
   ...
   protected Object getData(ActivationDataFlavor aFlavor, DataSource ds) throws IOException {
      if (aFlavor.getRepresentationClass() == String.class) {
         ...
         return aFlavor.getRepresentationClass() == StreamSource.class ? new StreamSource(ds.getInputStream()) : null;
      }
   }
   ...
   public void writeTo(Object obj, String mimeType, OutputStream os) throws IOException {
      if (!this.isXmlType(mimeType)) {
         ...
         try {
            ...
            ioex = new IOException("Unable to run the JAXP transformer on a stream " + var6.getMessage());
            ioex.initCause(var6);
            throw ioex;
         } catch (RuntimeException var7) {
            ioex = new IOException("Unable to run the JAXP transformer on a stream " + var7.getMessage());
            ioex.initCause(var7);
            throw ioex;
         }
      }
