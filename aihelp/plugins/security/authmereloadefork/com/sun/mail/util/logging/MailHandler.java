...
public class MailHandler extends Handler {
      ...
      if (expect == 0) {
         ...
         for(int i = 0; i < expect; ++i) {
            if (this.attachmentNames[i] == null) {
               this.attachmentNames[i] = MailHandler.TailNameFormatter.of(this.toString(this.attachmentFormatters[i]));
            }
         }
      }
   ...
   private void initFormatter(String p) {
      ...
      try {
         ...
         if (hasValue(name)) {
            ...
            if (!(f instanceof MailHandler.TailNameFormatter)) {
               this.formatter = f;
            } else {
   ...
   private boolean isPushable(LogRecord record) {
      ...
      if (value != offValue && record.getLevel().intValue() >= value) {
         ...
         if (push == null) {
            ...
            return (match != -1 || this.getFilter() != push) && (match < 0 || this.attachmentFilters[match] != push) ? push.isLoggable(record) : true;
         }
      } else {
         return false;
      }
   ...
   private void send(Message msg, boolean priority, int code) {
      try {
         ...
         this.reportError((Message)msg, var10, code);
      } catch (Exception var11) {
         this.reportError(msg, var11, code);
      }
   ...
   private Message writeLogRecords(int code) {
      try {
         ...
         this.reportError((String)var12.getMessage(), var12, code);
      } catch (Exception var13) {
         this.reportError(var13.getMessage(), var13, code);
      }
   ...
   private void verifySettings0(Session session, String verify) {
      ...
      if (!"local".equals(verify) && !"remote".equals(verify) && !"limited".equals(verify) && !"resolve".equals(verify) && !"login".equals(verify)) {
         this.reportError((String)"Verify must be 'limited', local', 'resolve', 'login', or 'remote'.", new IllegalArgumentException(verify), 4);
      } else {
         ...
         if (!"limited".equals(verify)) {
            ...
            try {
               ...
               UnsupportedEncodingException UEE = new UnsupportedEncodingException(var73.toString());
               UEE.initCause(var73);
         ...
         try {
            ...
            } catch (MessagingException var80) {
               MessagingException protocol = var80;
               ccl = this.getAndSetContextClassLoader(MAILHANDLER_LOADER);

               try {
                  t = session.getTransport();
               } catch (MessagingException var71) {
                  throw attach(protocol, var71);
               } finally {
                  this.getAndSetContextClassLoader(ccl);
               }
            }
            ...
            if (!"remote".equals(verify) && !"login".equals(verify)) {
               ...
               if ("resolve".equals(verify)) {
                  ...
                  } catch (IOException | RuntimeException var69) {
                     ME = new MessagingException(msg, var69);
               ...
               try {
                  try {
                     ...
                     try {
                        t.close();
                     } catch (MessagingException var70) {
                        closed = var70;
                     }
               ...
               } catch (SendFailedException var78) {
                  Address[] recip = var78.getInvalidAddresses();
                  if (recip != null && recip.length != 0) {
                     this.setErrorContent(abort, verify, var78);
                     this.reportError((Message)abort, var78, 4);
                  }

                  recip = var78.getValidSentAddresses();
                  if (recip != null && recip.length != 0) {
                     this.reportUnexpectedSend(abort, verify, var78);
                  }
               } catch (MessagingException var79) {
                  if (!this.isMissingContent(abort, var79)) {
                     this.setErrorContent(abort, verify, var79);
                     this.reportError((Message)abort, var79, 4);
                  }
               }
            ...
            if (!"limited".equals(verify)) {
               ...
               try {
                  ...
                  verifyHost(local);
               } catch (IOException | RuntimeException var68) {
                  ME = new MessagingException(msg, var68);
                  this.setErrorContent(abort, verify, ME);
                  this.reportError((Message)abort, ME, 4);
               }
               ...
               try {
                  ...
                  ME = new MessagingException(msg, var76);
                  this.setErrorContent(abort, verify, ME);
            ...
            this.setErrorContent(abort, verify, var81);
            this.reportError((Message)abort, var81, 4);
         } catch (Exception var82) {
            this.setErrorContent(abort, verify, var82);
            this.reportError((Message)abort, var82, 4);
         }
   ...
   private String toMsgString(Throwable t) {
      if (t == null) {
         ...
         try {
            ...
            return t.toString() + ' ' + var38.toString();
         } catch (Exception var39) {
            return t.toString() + ' ' + var39.toString();
         }
      }
   }
      ...
      if (ccl != MailHandler.GetAndSetContext.NOT_MODIFIED) {
         try {
            ...
            if (ccl instanceof PrivilegedAction) {
               ...
               pa = new MailHandler.GetAndSetContext(ccl);
            }
   ...
   static {
      ...
      MAILHANDLER_LOADER = new MailHandler.GetAndSetContext(MailHandler.class);
   ...
   private static final class TailNameFormatter extends Formatter {
      ...
      static Formatter of(String name) {
         return new MailHandler.TailNameFormatter(name);
      }
   ...
   private static final class GetAndSetContext implements PrivilegedAction<Object> {
      public static final Object NOT_MODIFIED = MailHandler.GetAndSetContext.class;
   ...
   private static final class DefaultAuthenticator extends Authenticator {
      ...
      static Authenticator of(String pass) {
         return new MailHandler.DefaultAuthenticator(pass);
      }
