...
final class OldMessageKeysMigrater {
   ...
   static boolean migrateOldPaths(PropertyReader reader, MessageKeyConfigurationData configurationData) {
      ...
      for(Iterator var3 = KEYS_TO_OLD_PATH.entrySet().iterator(); var3.hasNext(); wasPropertyMoved |= moveIfApplicable(reader, configurationData, (MessageKey)migrationEntry.getKey(), (String)migrationEntry.getValue())) {
         migrationEntry = (Entry)var3.next();
      }
   ...
   private static String replaceOldPlaceholders(MessageKey key, String text) {
      Map<String, String> replacements = (Map)PLACEHOLDER_REPLACEMENTS.get(key);
      if (replacements == null) {
         ...
         Entry replacement;
         for(Iterator var4 = replacements.entrySet().iterator(); var4.hasNext(); newText = newText.replace((CharSequence)replacement.getKey(), (CharSequence)replacement.getValue())) {
            replacement = (Entry)var4.next();
         }
