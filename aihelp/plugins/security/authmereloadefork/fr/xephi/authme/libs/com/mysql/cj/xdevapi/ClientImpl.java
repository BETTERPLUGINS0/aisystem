...
import fr.xephi.authme.libs.com.mysql.cj.conf.BooleanPropertyDefinition;
import fr.xephi.authme.libs.com.mysql.cj.conf.ConnectionUrl;
import fr.xephi.authme.libs.com.mysql.cj.conf.DefaultPropertySet;
import fr.xephi.authme.libs.com.mysql.cj.conf.HostInfo;
import fr.xephi.authme.libs.com.mysql.cj.conf.IntegerPropertyDefinition;
...
import fr.xephi.authme.libs.com.mysql.cj.protocol.x.XProtocolError;
...
public class ClientImpl implements Client, Protocol.ProtocolEventListener {
   ...
   public Session getSession() {
      if (this.isClosed) {
         ...
         synchronized(this) {
            List<ClientImpl.PooledXProtocol> toCloseAndRemove = (List)this.idleProtocols.stream().filter((p) -> {
               return !p.isHostInfoValid(hostsList);
            }).collect(Collectors.toList());
            Stream var10000 = toCloseAndRemove.stream().peek(ClientImpl.PooledXProtocol::realClose);
            BlockingQueue var10001 = this.idleProtocols;
            var10001.getClass();
            var10000 = (Stream)var10000.peek(var10001::remove).map(ClientImpl.PooledXProtocol::getHostInfo).sequential();
   ...
   private void removeActivePooledXProtocol(ClientImpl.PooledXProtocol prot) {
      ...
      while(var3.hasNext()) {
         WeakReference<ClientImpl.PooledXProtocol> wps = (WeakReference)var3.next();
         if (wps != null) {
            ClientImpl.PooledXProtocol as = (ClientImpl.PooledXProtocol)wps.get();
            if (as == prot) {
               wprot = wps;
