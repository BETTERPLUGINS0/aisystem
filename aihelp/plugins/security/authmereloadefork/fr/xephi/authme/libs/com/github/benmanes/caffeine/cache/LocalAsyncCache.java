...
interface LocalAsyncCache<K, V> extends AsyncCache<K, V> {
   ...
   default CompletableFuture<V> get(@NonNull K key, @NonNull Function<? super K, ? extends V> mappingFunction) {
      ...
      return this.get(key, (k1, executor) -> {
   ...
   default CompletableFuture<Map<K, V>> getAll(Iterable<? extends K> keys, Function<Iterable<? extends K>, Map<K, V>> mappingFunction) {
      Objects.requireNonNull(mappingFunction);
      return this.getAll(keys, (keysToLoad, executor) -> {
         return CompletableFuture.supplyAsync(() -> {
   ...
   default CompletableFuture<Map<K, V>> getAll(Iterable<? extends K> keys, BiFunction<Iterable<? extends K>, Executor, CompletableFuture<Map<K, V>>> mappingFunction) {
      ...
      if (proxies.isEmpty()) {
         ...
         LocalAsyncCache.AsyncBulkCompleter completer = new LocalAsyncCache.AsyncBulkCompleter(this.cache(), proxies);

         try {
            ((CompletableFuture)mappingFunction.apply(proxies.keySet(), this.cache().executor())).whenComplete(completer);
            return this.composeResult(futures);
         } catch (Throwable var9) {
            completer.accept((Map)null, (Throwable)var9);
            throw var9;
         }
      }
   }
      ...
      if (futures.isEmpty()) {
         ...
         return CompletableFuture.allOf(array).thenApply((ignored) -> {
            ...
            futures.forEach((key, future) -> {
               V value = future.getNow((Object)null);
               if (value != null) {
   ...
   public static final class AsMapView<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
      ...
      public boolean containsValue(Object value) {
         ...
         Iterator var2 = this.delegate.values().iterator();
         ...
         do {
            ...
            valueFuture = (CompletableFuture)var2.next();
         } while(!value.equals(Async.getIfReady(valueFuture)));
      ...
      public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
         ...
         do {
            ...
            future = new CompletableFuture[1];
            computed = (CompletableFuture)this.delegate.compute(key, (k, valueFuture) -> {
         ...
         } while(computed != future[0] && result == null);
            ...
            return new Iterator<Entry<K, V>>() {
               ...
               public boolean hasNext() {
                  while(this.cursor == null && this.iterator.hasNext()) {
                     ...
                     V value = Async.getIfReady((CompletableFuture)entry.getValue());
                     if (value != null) {
   ...
   public static final class AsyncBulkCompleter<K, V> implements BiConsumer<Map<K, V>, Throwable> {
      ...
      public void accept(@Nullable Map<K, V> result, @Nullable Throwable error) {
         ...
         if (result == null) {
            ...
            Iterator var5 = this.proxies.entrySet().iterator();

            while(var5.hasNext()) {
               Entry<K, CompletableFuture<V>> entry = (Entry)var5.next();
               this.cache.remove(entry.getKey(), entry.getValue());
               ((CompletableFuture)entry.getValue()).obtrudeException((Throwable)error);
            }

            this.cache.statsCounter().recordLoadFailure(loadTime);
            if (!(error instanceof CancellationException) && !(error instanceof TimeoutException)) {
               LocalAsyncCache.logger.log(Level.WARNING, "Exception thrown during asynchronous load", (Throwable)error);
            }
         } else {
