...
public class LinkedListMultimap<K, V> extends AbstractMultimap<K, V> implements ListMultimap<K, V>, Serializable {
   ...
   private LinkedListMultimap.Node<K, V> addNode(@ParametricNullness K key, @ParametricNullness V value, @CheckForNull LinkedListMultimap.Node<K, V> nextSibling) {
      ...
      if (this.head == null) {
         ...
         if (nextSibling == null) {
            ((LinkedListMultimap.Node)Objects.requireNonNull(this.tail)).next = node;
            ...
            keyList = (LinkedListMultimap.KeyList)Objects.requireNonNull((LinkedListMultimap.KeyList)this.keyToKeyList.get(key));
            ++keyList.count;
   ...
   private void removeNode(LinkedListMultimap.Node<K, V> node) {
      ...
      if (node.previousSibling == null && node.nextSibling == null) {
         keyList = (LinkedListMultimap.KeyList)Objects.requireNonNull((LinkedListMultimap.KeyList)this.keyToKeyList.remove(node.key));
         keyList.count = 0;
         ++this.modCount;
      } else {
         keyList = (LinkedListMultimap.KeyList)Objects.requireNonNull((LinkedListMultimap.KeyList)this.keyToKeyList.get(node.key));
         --keyList.count;
         if (node.previousSibling == null) {
            keyList.head = (LinkedListMultimap.Node)Objects.requireNonNull(node.nextSibling);
         } else {
            node.previousSibling.nextSibling = node.nextSibling;
         }

         if (node.nextSibling == null) {
            keyList.tail = (LinkedListMultimap.Node)Objects.requireNonNull(node.previousSibling);
         } else {
   ...
   List<V> createValues() {
      class ValuesImpl extends AbstractSequentialList<V> {
         ...
         public ListIterator<V> listIterator(int index) {
            ...
            return new TransformedListIterator<Entry<K, V>, V>(this, nodeItr) {
               @ParametricNullness
   ...
   List<Entry<K, V>> createEntries() {
      class EntriesImpl extends AbstractSequentialList<Entry<K, V>> {
         ...
         public void forEach(Consumer<? super Entry<K, V>> action) {
            ...
            for(LinkedListMultimap.Node node = LinkedListMultimap.this.head; node != null; node = node.next) {
               action.accept(node);
            }
