   ...
   public void configurePostHandshake(boolean dontCheckServerMatch) {
      ...
      if (sessionCharsetName != null) {
         ...
         if (dontCheckServerMatch || isCharsetDifferent || isCollationDifferent) {
            this.session.getProtocol().sendCommand(this.getCommandBuilder().buildComQuery((NativePacketPayload)null, (String)("SET NAMES " + sessionCharsetName + sessionCollationClause)), false, 0);
      ...
      if (!StringUtils.isNullOrEmpty(characterSetResultsValue) && !"null".equalsIgnoreCase(characterSetResultsValue)) {
         ...
         if (!resultsCharsetName.equalsIgnoreCase(sessionResultsCharset)) {
            this.session.getProtocol().sendCommand(this.getCommandBuilder().buildComQuery((NativePacketPayload)null, (String)("SET character_set_results = " + resultsCharsetName)), false, 0);
         ...
         if (!StringUtils.isNullOrEmpty(sessionResultsCharset) && !"NULL".equalsIgnoreCase(sessionResultsCharset)) {
            this.session.getProtocol().sendCommand(this.getCommandBuilder().buildComQuery((NativePacketPayload)null, (String)"SET character_set_results = NULL"), false, 0);
   ...
   private boolean characterSetNamesMatches(String mysqlEncodingName) {
      ...
      if (mysqlEncodingName != null) {
         ...
         if (!res && (aliases = CharsetMapping.getStaticMysqlCharsetAliasesByName(mysqlEncodingName)) != null) {
            Iterator var4 = aliases.iterator();
      ...
      if (customCollationIndexToCharsetName == null && (Boolean)this.session.getPropertySet().getBooleanProperty(PropertyKey.detectCustomCollations).getValue()) {
         ...
         try {
            ...
            while((r = (Row)rs.getRows().next()) != null) {
               ...
               if (collationIndex >= 1024 || collationIndex != getStaticCollationIndexForCollationName(collationName) || !charsetName.equals(getStaticMysqlCharsetNameForCollationIndex(collationIndex))) {
                  ...
                  if (isDefault || !((Map)customCharsetNameToCollationIndex).containsKey(charsetName) && CharsetMapping.getStaticCollationIndexForMysqlCharsetName(charsetName) == 0) {
