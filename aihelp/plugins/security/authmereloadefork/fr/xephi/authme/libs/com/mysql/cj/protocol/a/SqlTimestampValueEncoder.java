...
public class SqlTimestampValueEncoder extends AbstractValueEncoder {
   ...
   public String getString(BindValue binding) {
      ...
      case NULL:
         return "null";
      case DATE:
         return binding.getCalendar() != null ? TimeUtil.getSimpleDateFormat("''yyyy-MM-dd''", binding.getCalendar()).format(new Date(((java.util.Date)binding.getValue()).getTime())) : TimeUtil.getSimpleDateFormat((SimpleDateFormat)null, "''yyyy-MM-dd''", this.serverSession.getDefaultTimeZone()).format(new Date(((java.util.Date)binding.getValue()).getTime()));
      case DATETIME:
      case TIMESTAMP:
      case CHAR:
      case VARCHAR:
      ...
      case TIME:
         ...
         sb.append(this.adjustLocalTime(((Timestamp)binding.getValue()).toLocalDateTime().toLocalTime(), binding.getField()).format(TimeUtil.TIME_FORMATTER_WITH_OPTIONAL_MICROS));
         sb.append("'");
         return sb.toString();
      default:
   ...
   public void encodeAsBinary(Message msg, BindValue binding) {
      ...
      case TIME:
         ...
         calendar.setTime(t);
         this.writeTime(msg, InternalTime.from(calendar, this.adjustTimestamp((Timestamp)((Timestamp)binding.getValue()).clone(), binding.getField(), binding.keepOrigNanos()).getNanos()));
         return;
      default:
         throw (WrongArgumentException)ExceptionFactory.createException(WrongArgumentException.class, Messages.getString("PreparedStatement.67", new Object[]{binding.getValue().getClass().getName(), binding.getMysqlType().toString()}), this.exceptionInterceptor);
      }
