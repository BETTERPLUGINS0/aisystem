...
import fr.xephi.authme.libs.com.mysql.cj.jdbc.ClientPreparedStatement;
...
public class UpdatableResultSet extends ResultSetImpl {
         ...
         synchronized(this.checkClosed().getConnectionMutex()) {
            if (!this.isUpdatable) {
               ...
               for(int i = 0; i < numKeys; ++i) {
                  int index = (Integer)this.primaryKeyIndices.get(i);
   ...
   private void setParamValue(ClientPreparedStatement ps, int psIdx, Row row, int rsIdx, Field field) throws SQLException {
      ...
      if (val == null) {
         ...
         case DATE:
            ...
            break;
         case TIMESTAMP:
            ps.setObject(psIdx, this.getObject(rsIdx + 1, Timestamp.class), MysqlType.TIMESTAMP, field.getDecimals());
   ...
   private void refreshRow(ClientPreparedStatement updateInsertStmt, Row rowToRefresh) throws SQLException {
      ...
      for(int i = 0; i < numKeys; ++i) {
         byte[] dataFrom = null;
         i = (Integer)this.primaryKeyIndices.get(i);
         if (!this.doingUpdates && !this.onInsertRow) {
            ...
            if (!updateInsertStmt.isNull(i + 1) && dataFrom.length != 0) {
               this.refresher.getQueryBindings().setFromBindValue(i, updateInsertStmt.getQueryBindings().getBindValues()[i]);
            } else {
               this.setParamValue(this.refresher, i + 1, this.thisRow, i, this.getMetadata().getFields()[i]);
            }
         }
      ...
      try {
         ...
         for(i = 0; i < numCols; ++i) {
            ...
            rowToRefresh.setBytes(i, val != null && !rs.wasNull() ? val : null);
         }
      } finally {
         if (rs != null) {
            try {
   ...
   protected void syncUpdate() throws SQLException {
      ...
      i = this.primaryKeyIndices.size();

      for(int i = 0; i < i; ++i) {
         int idx = (Integer)this.primaryKeyIndices.get(i);
