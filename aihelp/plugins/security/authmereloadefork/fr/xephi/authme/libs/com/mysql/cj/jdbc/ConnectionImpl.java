...
import fr.xephi.authme.libs.com.mysql.cj.CacheAdapter;
import fr.xephi.authme.libs.com.mysql.cj.CacheAdapterFactory;
import fr.xephi.authme.libs.com.mysql.cj.LicenseConfiguration;
import fr.xephi.authme.libs.com.mysql.cj.Messages;
import fr.xephi.authme.libs.com.mysql.cj.NativeSession;
import fr.xephi.authme.libs.com.mysql.cj.NoSubInterceptorWrapper;
...
import fr.xephi.authme.libs.com.mysql.cj.exceptions.ExceptionInterceptorChain;
import fr.xephi.authme.libs.com.mysql.cj.exceptions.PasswordExpiredException;
...
import fr.xephi.authme.libs.com.mysql.cj.jdbc.ha.MultiHostMySQLConnection;
import fr.xephi.authme.libs.com.mysql.cj.jdbc.interceptors.ConnectionLifecycleInterceptor;
import fr.xephi.authme.libs.com.mysql.cj.jdbc.result.CachedResultSetMetaData;
import fr.xephi.authme.libs.com.mysql.cj.jdbc.result.CachedResultSetMetaDataImpl;
...
import fr.xephi.authme.libs.com.mysql.cj.jdbc.result.UpdatableResultSet;
import fr.xephi.authme.libs.com.mysql.cj.log.StandardLogger;
...
import fr.xephi.authme.libs.com.mysql.cj.protocol.SocksProxySocketFactory;
import fr.xephi.authme.libs.com.mysql.cj.protocol.a.NativePacketPayload;
import fr.xephi.authme.libs.com.mysql.cj.protocol.a.NativeProtocol;
...
public class ConnectionImpl implements JdbcConnection, Session.SessionEventListener, Serializable {
   ...
   public void unSafeQueryInterceptors() throws SQLException {
      try {
         Stream var10001 = this.queryInterceptors.stream();
         NoSubInterceptorWrapper.class.getClass();
         this.queryInterceptors = (List)var10001.map(NoSubInterceptorWrapper.class::cast).map(NoSubInterceptorWrapper::getUnderlyingInterceptor).collect(Collectors.toCollection(LinkedList::new));
   ...
   public void initializeSafeQueryInterceptors() throws SQLException {
      try {
         this.queryInterceptors = (List)Util.loadClasses(QueryInterceptor.class, this.propertySet.getStringProperty(PropertyKey.queryInterceptors).getStringValue(), "MysqlIo.BadQueryInterceptor", this.getExceptionInterceptor()).stream().map((o) -> {
   ...
   private void createPreparedStatementCaches() throws SQLException {
      synchronized(this.getConnectionMutex()) {
         ...
         CacheAdapterFactory<String, QueryInfo> cacheFactory = (CacheAdapterFactory)Util.getInstance(CacheAdapterFactory.class, queryInfoCacheFactory, (Class[])null, (Object[])null, this.getExceptionInterceptor());
         this.queryInfoCache = cacheFactory.getInstance(this, this.origHostInfo.getDatabaseUrl(), cacheSize, (Integer)this.prepStmtCacheSqlLimit.getValue());
         if ((Boolean)this.useServerPrepStmts.getValue()) {
            this.serverSideStatementCheckCache = new LRUCache(cacheSize);
            this.serverSideStatementCache = new LRUCache<ConnectionImpl.CompoundCacheKey, ServerPreparedStatement>(cacheSize) {
               private static final long serialVersionUID = 7692318650375988114L;

               protected boolean removeEldestEntry(Entry<ConnectionImpl.CompoundCacheKey, ServerPreparedStatement> eldest) {
   ...
   private void initializePropsFromServer() throws SQLException {
      ...
      if (connectionInterceptorClasses != null) {
         try {
            this.connectionLifecycleInterceptors = (List)Util.loadClasses(ConnectionLifecycleInterceptor.class, connectionInterceptorClasses, "Connection.badLifecycleInterceptor", this.getExceptionInterceptor()).stream().map((i) -> {
               return i.init(this, this.props, this.session.getLog());
            }).collect(Collectors.toCollection(LinkedList::new));
      ...
      try {
         ...
         if (!(Boolean)this.propertySet.getBooleanProperty(PropertyKey.cacheCallableStmts).getValue()) {
            ...
            synchronized(this.parsedCallableStatementCache) {
               ConnectionImpl.CompoundCacheKey key = new ConnectionImpl.CompoundCacheKey(this.getDatabase(), sql);
      ...
      try {
         synchronized(this.getConnectionMutex()) {
            if ((Boolean)this.cachePrepStmts.getValue() && pstmt.isPoolable()) {
               synchronized(this.serverSideStatementCache) {
                  Object oldServerPrepStmt = this.serverSideStatementCache.put(new ConnectionImpl.CompoundCacheKey(pstmt.getCurrentDatabase(), ((PreparedQuery)pstmt.getQuery()).getOriginalSql()), (ServerPreparedStatement)pstmt);
   ...
   public void decachePreparedStatement(JdbcPreparedStatement pstmt) throws SQLException {
      try {
         synchronized(this.getConnectionMutex()) {
            if ((Boolean)this.cachePrepStmts.getValue()) {
               synchronized(this.serverSideStatementCache) {
                  this.serverSideStatementCache.remove(new ConnectionImpl.CompoundCacheKey(pstmt.getCurrentDatabase(), ((PreparedQuery)pstmt.getQuery()).getOriginalSql()));
               }
            }
      ...
      try {
         synchronized(this.getConnectionMutex()) {
            ...
            if (executor == null) {
               ...
               this.checkClosed();
               executor.execute(new ConnectionImpl.NetworkTimeoutSetter(this, milliseconds));
            }
         }
      } catch (CJException var8) {
         throw SQLExceptionsMapping.translateException(var8, this.getExceptionInterceptor());
      ...
      try {
         synchronized(this.getConnectionMutex()) {
            if (this.infoProvider == null) {
               ...
               try {
                  this.infoProvider = (ClientInfoProvider)Util.getInstance(ClientInfoProvider.class, clientInfoProvider, (Class[])null, (Object[])null, this.getExceptionInterceptor());
               } catch (CJException var8) {
                  if (!ClassNotFoundException.class.isInstance(var8.getCause())) {
                     throw SQLExceptionsMapping.translateException(var8, this.getExceptionInterceptor());
                  }

                  try {
                     this.infoProvider = (ClientInfoProvider)Util.getInstance(ClientInfoProvider.class, "fr.xephi.authme.libs.com.mysql.cj.jdbc." + clientInfoProvider, (Class[])null, (Object[])null, this.getExceptionInterceptor());
                  } catch (CJException var7) {
                     throw SQLExceptionsMapping.translateException(var8, this.getExceptionInterceptor());
                  }
               }
   ...
   static class CompoundCacheKey {
      ...
      CompoundCacheKey(String partOne, String partTwo) {
         ...
         int hc = 17;
         int hc = 31 * hc + (this.componentOne != null ? this.componentOne.hashCode() : 0);
