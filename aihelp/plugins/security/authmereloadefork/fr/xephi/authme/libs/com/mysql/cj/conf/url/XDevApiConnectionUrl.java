...
import fr.xephi.authme.libs.com.mysql.cj.conf.HostsListView;
   ...
   public List<HostInfo> getHostsList(HostsListView view) {
      if (this.prioritySorted) {
         if (this.hasDuplicatedPriorities) {
            ...
            this.hosts = (List)hostsByPriority.entrySet().stream().sorted(Comparator.comparing(Entry::getKey).reversed()).map(Entry::getValue).peek(Collections::shuffle).flatMap(Collection::stream).collect(Collectors.toList());
         }
      } else {
