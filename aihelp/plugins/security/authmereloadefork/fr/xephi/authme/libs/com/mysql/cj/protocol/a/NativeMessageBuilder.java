      ...
      synchronized(this) {
         ...
         if (this.supportsQueryAttributes) {
            if (queryAttributesBindings.getCount() > 0) {
               ...
               for(ensurePacketSize = 0; ensurePacketSize < queryAttributesBindings.getCount(); ++ensurePacketSize) {
                  if (queryAttributesBindings.getAttributeValue(ensurePacketSize).isNull()) {
         ...
         boolean useStreamLengths = (Boolean)sess.getPropertySet().getBooleanProperty(PropertyKey.useStreamLengthsInPrepStmts).getValue();
         ensurePacketSize = 0;
