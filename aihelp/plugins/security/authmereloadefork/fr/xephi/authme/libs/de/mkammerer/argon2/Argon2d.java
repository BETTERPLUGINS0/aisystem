...
class Argon2d extends BaseArgon2 {
   ...
   protected int callLibraryHash(byte[] pwd, byte[] salt, JnaUint32 iterations, JnaUint32 memory, JnaUint32 parallelism, byte[] encoded) {
      return Argon2Library.INSTANCE.argon2d_hash_encoded(iterations, memory, parallelism, pwd, new Size_t((long)pwd.length), salt, new Size_t((long)salt.length), new Size_t((long)this.getDefaultHashLength()), encoded, new Size_t((long)encoded.length));
   }
   ...
   protected int callLibraryRawHash(byte[] pwd, byte[] salt, JnaUint32 iterations, JnaUint32 memory, JnaUint32 parallelism, byte[] hash) {
      return Argon2Library.INSTANCE.argon2d_hash_raw(iterations, memory, parallelism, pwd, new Size_t((long)pwd.length), salt, new Size_t((long)salt.length), hash, new Size_t((long)hash.length));
   }

   protected int callLibraryVerify(byte[] encoded, byte[] pwd) {
      return Argon2Library.INSTANCE.argon2d_verify(encoded, pwd, new Size_t((long)pwd.length));
   }
