...
public class PGInterval extends PGobject implements Serializable, Cloneable {
   ...
   public void setValue(@Nullable String value) throws SQLException {
      ...
      if (value == null) {
         this.setValue(0, 0, 0, 0, 0, 0.0D);
         this.isNull = true;
      } else {
         boolean postgresFormat = !value.startsWith("@");
         if (value.startsWith("P")) {
            this.parseISO8601Format(value);
         } else if (!postgresFormat && value.length() == 3 && value.charAt(2) == '0') {
            this.setValue(0, 0, 0, 0, 0, 0.0D);
         } else {
            int years = 0;
            int months = 0;
            ...
            try {
               ...
               throw new PSQLException(GT.tr("Conversion of interval failed"), PSQLState.NUMERIC_CONSTANT_OUT_OF_RANGE, var17);
            }
   ...
   public double getSeconds() {
      return (double)this.wholeSeconds + (double)this.microSeconds / 1000000.0D;
   }
   ...
   public void setSeconds(double seconds) {
      ...
      this.microSeconds = (int)Math.round((seconds - (double)this.wholeSeconds) * 1000000.0D);
   }
   ...
   public boolean equals(@Nullable Object obj) {
      if (obj == null) {
         ...
         return true;
      } else if (!(obj instanceof PGInterval)) {
         return false;
      } else {
   ...
   public int hashCode() {
      return this.isNull ? 0 : (((((((248 + this.microSeconds) * 31 + this.wholeSeconds) * 31 + this.minutes) * 31 + this.hours) * 31 + this.days) * 31 + this.months) * 31 + this.years) * 31;
   }
