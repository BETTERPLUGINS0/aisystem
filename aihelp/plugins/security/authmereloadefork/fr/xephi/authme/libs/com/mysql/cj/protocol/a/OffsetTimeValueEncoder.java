...
public class OffsetTimeValueEncoder extends AbstractValueEncoder {
   public String getString(BindValue binding) {
      ...
      case LONGTEXT:
         sb = new StringBuilder("'");
         sb.append(((OffsetTime)binding.getValue()).format((Boolean)this.sendFractionalSeconds.getValue() && ((OffsetTime)binding.getValue()).getNano() > 0 ? TimeUtil.TIME_FORMATTER_WITH_NANOS_WITH_OFFSET : TimeUtil.TIME_FORMATTER_NO_FRACT_WITH_OFFSET));
         sb.append("'");
         return sb.toString();
      default:
   ...
   public void encodeAsBinary(Message msg, BindValue binding) {
      ...
      case MEDIUMTEXT:
      case LONGTEXT:
         intoPacket.writeBytes(NativeConstants.StringSelfDataType.STRING_LENENC, StringUtils.getBytes(((OffsetTime)binding.getValue()).format((Boolean)this.sendFractionalSeconds.getValue() && ((OffsetTime)binding.getValue()).getNano() > 0 ? TimeUtil.TIME_FORMATTER_WITH_NANOS_WITH_OFFSET : TimeUtil.TIME_FORMATTER_NO_FRACT_WITH_OFFSET), (String)this.charEncoding.getValue()));
         return;
      default:
         throw (WrongArgumentException)ExceptionFactory.createException(WrongArgumentException.class, Messages.getString("PreparedStatement.67", new Object[]{binding.getValue().getClass().getName(), binding.getMysqlType().toString()}), this.exceptionInterceptor);
      }
