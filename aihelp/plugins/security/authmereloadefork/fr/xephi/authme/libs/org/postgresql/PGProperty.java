...
public enum PGProperty {
   ...
   AUTHENTICATION_PLUGIN_CLASS_NAME("authenticationPluginClassName", (String)null, "Name of class which implements AuthenticationPlugin"),
   ...
   LOCAL_SOCKET_ADDRESS("localSocketAddress", (String)null, "Local Socket address, if set bind the client side of the socket to this address"),
   /** @deprecated */
   @Deprecated
   ...
   SERVICE("service", (String)null, "Service name to be searched in pg_service.conf resource"),
   ...
   SSL_FACTORY("sslfactory", "fr.xephi.authme.libs.org.postgresql.ssl.LibPQFactory", "Provide a SSLSocketFactory class when using SSL."),
   /** @deprecated */
   @Deprecated
   ...
   SSL_PASSWORD_CALLBACK("sslpasswordcallback", (String)null, "A class, implementing javax.security.auth.callback.CallbackHandler that can handle PasswordCallback for the ssl password."),
   ...
   private PGProperty(String name, @Nullable String defaultValue, String description) {
      this(name, defaultValue, description, false);
   }

   private PGProperty(String name, @Nullable String defaultValue, String description, boolean required) {
      this(name, defaultValue, description, required, (String[])null);
   }

   private PGProperty(String name, @Nullable String defaultValue, String description, boolean required, @Nullable String[] choices) {
      this.name = name;
   ...
   public int getInt(Properties properties) throws PSQLException {
      ...
      try {
         ...
         throw new PSQLException(GT.tr("{0} parameter value must be an integer but was: {1}", this.getName(), value), PSQLState.INVALID_PARAMETER_VALUE, var4);
      }
   }
   ...
   // $FF: synthetic method
   private static PGProperty[] $values() {
      return new PGProperty[]{ADAPTIVE_FETCH, ADAPTIVE_FETCH_MAXIMUM, ADAPTIVE_FETCH_MINIMUM, ALLOW_ENCODING_CHANGES, APPLICATION_NAME, ASSUME_MIN_SERVER_VERSION, AUTHENTICATION_PLUGIN_CLASS_NAME, AUTOSAVE, BINARY_TRANSFER, BINARY_TRANSFER_DISABLE, BINARY_TRANSFER_ENABLE, CANCEL_SIGNAL_TIMEOUT, CLEANUP_SAVEPOINTS, CONNECT_TIMEOUT, CURRENT_SCHEMA, DATABASE_METADATA_CACHE_FIELDS, DATABASE_METADATA_CACHE_FIELDS_MIB, DEFAULT_ROW_FETCH_SIZE, DISABLE_COLUMN_SANITISER, ESCAPE_SYNTAX_CALL_MODE, GROUP_STARTUP_PARAMETERS, GSS_ENC_MODE, GSS_LIB, GSS_RESPONSE_TIMEOUT, HIDE_UNPRIVILEGED_OBJECTS, HOST_RECHECK_SECONDS, JAAS_APPLICATION_NAME, JAAS_LOGIN, KERBEROS_SERVER_NAME, LOAD_BALANCE_HOSTS, LOCAL_SOCKET_ADDRESS, LOGGER_FILE, LOGGER_LEVEL, LOGIN_TIMEOUT, LOG_SERVER_ERROR_DETAIL, LOG_UNCLOSED_CONNECTIONS, MAX_RESULT_BUFFER, OPTIONS, PASSWORD, PG_DBNAME, PG_HOST, PG_PORT, PREFER_QUERY_MODE, PREPARED_STATEMENT_CACHE_QUERIES, PREPARED_STATEMENT_CACHE_SIZE_MIB, PREPARE_THRESHOLD, PROTOCOL_VERSION, QUOTE_RETURNING_IDENTIFIERS, READ_ONLY, READ_ONLY_MODE, RECEIVE_BUFFER_SIZE, REPLICATION, REWRITE_BATCHED_INSERTS, SEND_BUFFER_SIZE, SERVICE, SOCKET_FACTORY, SOCKET_FACTORY_ARG, SOCKET_TIMEOUT, SSL, SSL_CERT, SSL_FACTORY, SSL_FACTORY_ARG, SSL_HOSTNAME_VERIFIER, SSL_KEY, SSL_MODE, SSL_PASSWORD, SSL_PASSWORD_CALLBACK, SSL_RESPONSE_TIMEOUT, SSL_ROOT_CERT, SSPI_SERVICE_CLASS, STRING_TYPE, TARGET_SERVER_TYPE, TCP_KEEP_ALIVE, TCP_NO_DELAY, UNKNOWN_LENGTH, USER, USE_SPNEGO, XML_FACTORY_FACTORY};
   }

   static {
      PGProperty[] var0 = values();
      int var1 = var0.length;

      for(int var2 = 0; var2 < var1; ++var2) {
         PGProperty prop = var0[var2];
