...
public class NativeQueryBindings implements QueryBindings {
   ...
   public synchronized void setNull(int parameterIndex) {
      BindValue binding = this.getBinding(parameterIndex, false);
      binding.setBinding((Object)null, MysqlType.NULL, this.numberOfExecutions, this.sendTypesToServer);
   }
   ...
   public void setTimestamp(int parameterIndex, Timestamp x, Calendar targetCalendar, Field field, MysqlType targetMysqlType) {
      if (x == null) {
         ...
         if (field == null && this.columnDefinition != null && parameterIndex <= this.columnDefinition.getFields().length && parameterIndex >= 0 && this.columnDefinition.getFields()[parameterIndex].getDecimals() > 0) {
   ...
   public void setObject(int parameterIndex, Object parameterObj, MysqlType targetMysqlType, int scaleOrLength) {
      if (parameterObj == null) {
         ...
         try {
            if (targetMysqlType != null && targetMysqlType != MysqlType.UNKNOWN && (!(parameterObj instanceof java.util.Date) || (Boolean)this.session.getPropertySet().getBooleanProperty(PropertyKey.treatUtilDateAsTimestamp).getValue())) {
               BindValue binding = this.getBinding(parameterIndex, false);
            ...
            throw ExceptionFactory.createException((String)(Messages.getString("PreparedStatement.17") + parameterObj.getClass().toString() + Messages.getString("PreparedStatement.18") + var6.getClass().getName() + Messages.getString("PreparedStatement.19") + var6.getMessage()), (Throwable)var6, (ExceptionInterceptor)this.session.getExceptionInterceptor());
         }
      }
   }
