...
import fr.xephi.authme.libs.com.mysql.cj.TransactionEventHandler;
...
import fr.xephi.authme.libs.com.mysql.cj.exceptions.ClosedOnExpiredPasswordException;
...
import fr.xephi.authme.libs.com.mysql.cj.jdbc.exceptions.MysqlDataTruncation;
import fr.xephi.authme.libs.com.mysql.cj.log.BaseMetricsHolder;
import fr.xephi.authme.libs.com.mysql.cj.log.Log;
import fr.xephi.authme.libs.com.mysql.cj.log.ProfilerEventHandler;
import fr.xephi.authme.libs.com.mysql.cj.protocol.AbstractProtocol;
...
import fr.xephi.authme.libs.com.mysql.cj.protocol.FullReadInputStream;
import fr.xephi.authme.libs.com.mysql.cj.protocol.Message;
import fr.xephi.authme.libs.com.mysql.cj.protocol.MessageReader;
import fr.xephi.authme.libs.com.mysql.cj.protocol.MessageSender;
...
import fr.xephi.authme.libs.com.mysql.cj.protocol.ProtocolEntityReader;
...
import fr.xephi.authme.libs.com.mysql.cj.util.LazyString;
import fr.xephi.authme.libs.com.mysql.cj.util.StringUtils;
import fr.xephi.authme.libs.com.mysql.cj.util.TestUtils;
...
public class NativeProtocol extends AbstractProtocol<NativePacketPayload> implements Protocol<NativePacketPayload>, RuntimeProperty.RuntimePropertyListener {
   ...
   public Supplier<ValueEncoder> getValueEncoderSupplier(Object obj) {
      if (obj == null) {
         ...
         Supplier<ValueEncoder> res = (Supplier)DEFAULT_ENCODERS.get(obj.getClass());
         if (res == null) {
            Optional<Supplier<ValueEncoder>> mysqlType = DEFAULT_ENCODERS.entrySet().stream().filter((m) -> {
               return ((Class)m.getKey()).isAssignableFrom(obj.getClass());
            }).map((m) -> {
               return (Supplier)m.getValue();
            }).findFirst();
            if (mysqlType.isPresent()) {
               res = (Supplier)mysqlType.get();
            }
         }
      ...
      if (supportsQueryAttributes) {
         ...
         if (queryAttributes != null && queryAttributes.getCount() > 0) {
            ...
            for(int i = 0; i < queryAttributes.getCount(); ++i) {
               ...
               packLength = (int)((long)packLength + (long)(2 + queryAttribute.getName().length()) + queryAttribute.getBinaryLength());
            }
         } else {
            packLength += 2;
         }
         ...
         if (safePathValue.length() == 0) {
            ...
            if ((Boolean)allowUrlInLocalInfile.getValue()) {
               try {
                  ...
                  InetAddress addr;
                  try {
                     addr = InetAddress.getByName(urlFromFileName.getHost());
