...
public final class MoreExecutors {
   ...
   static <T> T invokeAnyImpl(ListeningExecutorService executorService, Collection<? extends Callable<T>> tasks, boolean timed, long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
         ...
         while(true) {
            ...
            if (f != null) {
               ...
               try {
                  ...
                  ee = new ExecutionException(var28);
               }
            }
         }
      } finally {
         if (var26) {
            Iterator var21 = futures.iterator();

            while(var21.hasNext()) {
               Future<T> f = (Future)var21.next();
               f.cancel(true);
            }
      ...
      Iterator var18 = futures.iterator();

      while(var18.hasNext()) {
         Future<T> f = (Future)var18.next();
         f.cancel(true);
      }
   ...
   private static final class ScheduledListeningDecorator extends MoreExecutors.ListeningDecorator implements ListeningScheduledExecutorService {
      ...
      private static final class NeverSuccessfulListenableFutureTask extends AbstractFuture.TrustedFuture<Void> implements Runnable {
         ...
         protected String pendingToString() {
            ...
            return (new StringBuilder(7 + String.valueOf(var1).length())).append("task=[").append(var1).append("]").toString();
         }
      }
   ...
   private static class ListeningDecorator extends AbstractListeningExecutorService {
      ...
      public final String toString() {
         ...
         String var2 = String.valueOf(this.delegate);
   ...
   static class Application {
      ...
      final void addDelayedShutdownHook(final ExecutorService service, final long terminationTimeout, final TimeUnit timeUnit) {
         ...
         String var5 = String.valueOf(service);
         this.addShutdownHook(MoreExecutors.newThread((new StringBuilder(24 + String.valueOf(var5).length())).append("DelayedShutdownHook-for-").append(var5).toString(), new Runnable(this) {
            public void run() {
               try {
                  service.shutdown();
