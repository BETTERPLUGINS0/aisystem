...
final class MessageSchema<T> implements Schema<T> {
      ...
      for(int bufferIndex = 0; i < length; buffer[bufferIndex++] = presenceMaskShift << 20 | presenceFieldOffset) {
         ...
         if (fieldType >= 51) {
            ...
            if (next >= 55296) {
               ...
               for(oneofFieldType = 13; (next = info.charAt(i++)) >= '\ud800'; oneofFieldType += 13) {
                  result |= (next & 8191) << oneofFieldType;
               }

               next = result | next << oneofFieldType;
            }

            oneofFieldType = fieldType - 51;
            if (oneofFieldType != 9 && oneofFieldType != 17) {
               if (oneofFieldType == 12 && !isProto3) {
            ...
            shift = next * 2;
            Object o = messageInfoObjects[shift];
            ...
            if (o instanceof java.lang.reflect.Field) {
               ...
               messageInfoObjects[shift] = oneofField;
            }

            fieldOffset = (int)unsafe.objectFieldOffset(oneofField);
            ++shift;
            o = messageInfoObjects[shift];
            ...
            if (o instanceof java.lang.reflect.Field) {
               ...
               messageInfoObjects[shift] = oneofCaseField;
            }

            presenceFieldOffset = (int)unsafe.objectFieldOffset(oneofCaseField);
            presenceMaskShift = 0;
         } else {
            java.lang.reflect.Field field = reflectField(messageClass, (String)messageInfoObjects[var22++]);
         ...
         if (source == null) {
            ...
            if (!this.isFieldPresent(targetParent, pos)) {
               if (!isMutable(source)) {
                  ...
                  target = fieldSchema.newInstance();
                  fieldSchema.mergeFrom(target, source);
                  UNSAFE.putObject(targetParent, offset, target);
               }

               this.setFieldPresent(targetParent, pos);
            } else {
               target = UNSAFE.getObject(targetParent, offset);
               if (!isMutable(target)) {
         ...
         label513: {
            label514: {
               ...
               label515: {
                  ...
                  label516: {
                     ...
                     for(enumValue = this.checkInitializedCount; enumValue < this.repeatedFieldOffsetStart; ++enumValue) {
                        unknownFields = this.filterMapUnknownEnumValues(message, this.intArray[enumValue], unknownFields, unknownFieldSchema, message);
                     }
               ...
               for(typeAndOffset = this.checkInitializedCount; typeAndOffset < this.repeatedFieldOffsetStart; ++typeAndOffset) {
                  unknownFields = this.filterMapUnknownEnumValues(message, this.intArray[typeAndOffset], unknownFields, unknownFieldSchema, message);
               }
   ...
   private int parseOneofField(T message, byte[] data, int position, int limit, int tag, int number, int wireType, int typeAndOffset, int fieldType, long fieldOffset, int bufferPosition, ArrayDecoders.Registers registers) throws IOException {
      ...
      case 60:
         if (wireType == 2) {
            current = this.mutableOneofMessageFieldForMerge(message, number, bufferPosition);
   ...
   int parseProto2Message(T message, byte[] data, int position, int limit, int endGroup, ArrayDecoders.Registers registers) throws IOException {
      ...
      while(position < limit) {
         ...
         if (pos == -1) {
            ...
            if (fieldType <= 17) {
               ...
               case 9:
                  if (wireType == 2) {
                     current = this.mutableMessageFieldForMerge(message, pos);
      ...
      for(wireType = this.checkInitializedCount; wireType < this.repeatedFieldOffsetStart; ++wireType) {
         unknownFields = (UnknownFieldSetLite)this.filterMapUnknownEnumValues(message, this.intArray[wireType], unknownFields, this.unknownFieldSchema, message);
      }
   ...
   private boolean isFieldPresent(T message, int pos) {
      ...
      if (presenceFieldOffset == 1048575L) {
         ...
         typeAndOffset = 1 << (presenceMaskAndOffset >>> 20);
         return (UnsafeUtil.getInt(message, (long)(presenceMaskAndOffset & 1048575)) & typeAndOffset) != 0;
      }
   }
