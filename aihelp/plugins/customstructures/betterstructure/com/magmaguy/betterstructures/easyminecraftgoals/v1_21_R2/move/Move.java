 ...
 */
package com.magmaguy.betterstructures.easyminecraftgoals.v1_21_R2.move;
...
public class Move {
    public static boolean canReach(EntityCreature pathfinderMob, Location destination) {
        if (pathfinderMob.L() == null) {
            return true;
        }
        PathEntity path = pathfinderMob.L().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        if (path == null) {
            return true;
        }
        return path.j();
    ...
    public static boolean simpleMove(EntityCreature pathfinderMob, double speedModifier, Location destination) {
        PathEntity path = pathfinderMob.L().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        return pathfinderMob.L().a(path, speedModifier);
    }

    public static void doNotMove(EntityCreature pathfinderMob) {
        pathfinderMob.L().a((Entity)pathfinderMob, 0.0);
    }

    public static void universalMove(EntityInsentient mob, double speedModifier, Location destination) {
        double speed = mob.h(GenericAttributes.v) * 0.75;
        Vec3D movementInTick = new Vec3D(destination.getX(), destination.getY(), destination.getZ()).d(mob.du());
        ...
        Move.rotateHead((Entity)mob, destination.toVector(), new Vector(mob.du().d, mob.du().e, mob.du().f));
    }
