 ...
 */
package com.magmaguy.betterstructures.easyminecraftgoals.v1_20_R4.move;
...
public class Move {
    public static boolean canReach(EntityCreature pathfinderMob, Location destination) {
        if (pathfinderMob.K() == null) {
            return true;
        }
        PathEntity path = pathfinderMob.K().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        if (path == null) {
            return true;
        }
        return path.j();
    ...
    public static boolean simpleMove(EntityCreature pathfinderMob, double speedModifier, Location destination) {
        PathEntity path = pathfinderMob.K().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        return pathfinderMob.K().a(path, speedModifier);
    }

    public static void doNotMove(EntityCreature pathfinderMob) {
        pathfinderMob.K().a((Entity)pathfinderMob, 0.0);
    }

    public static void universalMove(EntityInsentient mob, double speedModifier, Location destination) {
        double speed = mob.g(GenericAttributes.r) * 0.75;
        Vec3D movementInTick = new Vec3D(destination.getX(), destination.getY(), destination.getZ()).d(mob.dn());
        ...
        Move.rotateHead((Entity)mob, destination.toVector(), new Vector(mob.dn().c, mob.dn().d, mob.dn().e));
    }
