 ...
 */
package com.magmaguy.betterstructures.easyminecraftgoals.v1_19_R3.move;
...
public class Move {
    public static boolean canReach(EntityCreature pathfinderMob, Location destination) {
        if (pathfinderMob.G() == null) {
            return true;
        }
        PathEntity path = pathfinderMob.G().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        if (path == null) {
            return true;
        }
        return path.j();
    ...
    public static boolean simpleMove(EntityCreature pathfinderMob, double speedModifier, Location destination) {
        PathEntity path = pathfinderMob.G().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        return pathfinderMob.G().a(path, speedModifier);
    }

    public static void doNotMove(EntityCreature pathfinderMob) {
        pathfinderMob.G().a((Entity)pathfinderMob, 0.0);
    }

    public static void universalMove(EntityInsentient mob, double speedModifier, Location destination) {
        double speed = mob.b(GenericAttributes.d) * 0.75;
        Vec3D movementInTick = new Vec3D(destination.getX(), destination.getY(), destination.getZ()).d(mob.de());
        ...
        Move.rotateHead((Entity)mob, destination.toVector(), new Vector(mob.de().c, mob.de().d, mob.de().e));
    }
