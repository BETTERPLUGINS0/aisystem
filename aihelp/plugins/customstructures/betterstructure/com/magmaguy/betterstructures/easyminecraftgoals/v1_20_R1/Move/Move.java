 ...
 */
package com.magmaguy.betterstructures.easyminecraftgoals.v1_20_R1.Move;
...
public class Move {
    public static boolean canReach(EntityCreature pathfinderMob, Location destination) {
        if (pathfinderMob.J() == null) {
            return true;
        }
        PathEntity path = pathfinderMob.J().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        if (path == null) {
            return true;
        }
        return path.j();
    ...
    public static boolean simpleMove(EntityCreature pathfinderMob, double speedModifier, Location destination) {
        PathEntity path = pathfinderMob.J().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        return pathfinderMob.J().a(path, speedModifier);
    }

    public static void doNotMove(EntityCreature pathfinderMob) {
        pathfinderMob.J().a((Entity)pathfinderMob, 0.0);
    }
    ...
    public static void universalMove(EntityInsentient mob, double speedModifier, Location destination) {
        ...
        Vec3D movementInTick = new Vec3D(destination.getX(), destination.getY(), destination.getZ()).d(mob.dg());
        ...
        Move.rotateHead((Entity)mob, destination.toVector(), new Vector(mob.dg().c, mob.dg().d, mob.dg().e));
    }
