 ...
 */
package com.magmaguy.betterstructures.easyminecraftgoals.v1_21_R3.move;
...
public class Move {
    public static boolean canReach(EntityCreature pathfinderMob, Location destination) {
        if (pathfinderMob.P() == null) {
            return true;
        }
        PathEntity path = pathfinderMob.P().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        if (path == null) {
            return true;
        }
        return path.j();
    ...
    public static boolean simpleMove(EntityCreature pathfinderMob, double speedModifier, Location destination) {
        PathEntity path = pathfinderMob.P().a(destination.getX(), destination.getY(), destination.getZ(), 0);
        return pathfinderMob.P().a(path, speedModifier);
    }

    public static void doNotMove(EntityCreature pathfinderMob) {
        pathfinderMob.P().a((Entity)pathfinderMob, 0.0);
    }
    ...
    public static void universalMove(EntityInsentient mob, double speedModifier, Location destination) {
        ...
        Vec3D movementInTick = new Vec3D(destination.getX(), destination.getY(), destination.getZ()).d(mob.dt());
        ...
        Move.rotateHead((Entity)mob, destination.toVector(), new Vector(mob.dt().d, mob.dt().e, mob.dt().f));
    }
