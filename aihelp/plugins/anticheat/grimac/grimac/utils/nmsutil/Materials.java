...
public final class Materials {
   ...
   public static boolean isWater(ClientVersion clientVersion, WrappedBlockState state) {
      ...
      if (modern && isWaterModern(state.getType())) {
         ...
         return !modern && isWaterLegacy(state.getType()) ? true : isWaterlogged(clientVersion, state);
      }
   }
   ...
   public static boolean isWaterSource(ClientVersion clientVersion, WrappedBlockState state) {
      if (isWaterlogged(clientVersion, state)) {
         return true;
      } else if (state.getType() == StateTypes.WATER && state.getLevel() == 0) {
         return true;
      } else {
         boolean modern = clientVersion.isNewerThanOrEquals(ClientVersion.V_1_13);
   ...
   public static boolean isWaterlogged(ClientVersion clientVersion, WrappedBlockState state) {
      if (clientVersion.isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
         ...
         if (clientVersion.isOlderThan(ClientVersion.V_1_16_2) && (type == StateTypes.LANTERN || type == StateTypes.SOUL_LANTERN)) {
            return false;
         } else if (clientVersion.isOlderThan(ClientVersion.V_1_17) && type == StateTypes.SMALL_DRIPLEAF) {
            return false;
         } else if (clientVersion.isOlderThan(ClientVersion.V_1_17) && BlockTags.RAILS.contains(type)) {
            return false;
         } else {
   ...
   public static StateType transformBucketMaterial(ItemType mat) {
      if (mat == ItemTypes.LAVA_BUCKET) {
         ...
         return isPlaceableWaterBucket(mat) ? StateTypes.WATER : null;
      }
   }
   ...
   public static boolean isSolidBlockingBlacklist(StateType mat, ClientVersion ver) {
      if (!mat.isBlocking()) {
         return true;
      } else if (!BlockTags.BANNERS.contains(mat)) {
         return false;
      } else {
   ...
   public static boolean isWaterIgnoringWaterlogged(ClientVersion clientVersion, WrappedBlockState state) {
      return clientVersion.isNewerThanOrEquals(ClientVersion.V_1_13) ? isWaterModern(state.getType()) : isWaterLegacy(state.getType());
   }
   ...
   public static boolean isClientSideOpenableDoor(StateType mat, ClientVersion ver) {
      if (!BlockTags.MOB_INTERACTABLE_DOORS.contains(mat)) {
         ...
         return COPPER_DOORS.contains(mat) ? ver.isNewerThanOrEquals(ClientVersion.V_1_20_3) : true;
      }
   }
   ...
   public static boolean isClientSideOpenableTrapdoor(StateType mat, ClientVersion ver) {
      if (!BlockTags.TRAPDOORS.contains(mat)) {
         return false;
      } else if (ver.isOlderThan(ClientVersion.V_1_8)) {
         return true;
      } else if (COPPER_TRAPDOORS.contains(mat)) {
   ...
   static {
      ...
      CLIENT_SIDE.addAll((Collection)BlockTags.TRAPDOORS.getStates().stream().filter((type) -> {
         return type != StateTypes.IRON_TRAPDOOR;
