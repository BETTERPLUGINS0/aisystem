...
public class CheckManagerListener extends PacketListenerAbstract {
   ...
   private static void handleBlockPlaceOrUseItem(PacketWrapper<?> packet, GrimPlayer player) {
      ...
      if (packet instanceof WrapperPlayClientPlayerBlockPlacement) {
         ...
         if ((!player.isSneaking || onlyAir) && place.getHand() == InteractionHand.MAIN_HAND) {
            ...
            BlockPlace blockPlace = new BlockPlace(player, place.getHand(), blockPosition, place.getFaceId(), place.getFace(), placedWith, WorldRayTrace.getNearestBlockHitResult(player, (StateType)null, true, false, false), place.getSequence());
      ...
      if (packet instanceof WrapperPlayClientPlayerBlockPlacement) {
         ...
         BlockPlace blockPlace = new BlockPlace(player, place.getHand(), blockPosition, place.getFaceId(), face, placedWith, WorldRayTrace.getNearestBlockHitResult(player, (StateType)null, true, false, false), place.getSequence());
         player.checkManager.onPostFlyingBlockPlace(blockPlace);
         blockPlace.isInside = (Boolean)place.getInsideBlock().orElse(false);
   ...
   private static void placeBucket(GrimPlayer player, InteractionHand hand, int sequence) {
      HitData data = WorldRayTrace.getNearestBlockHitResult(player, (StateType)null, true, false, true);
      if (data != null) {
         ...
         if (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_13)) {
            WrappedBlockState existing = blockPlace.getExistingBlockData();
            if (existing.hasProperty(StateValue.WATERLOGGED)) {
   ...
   private static void placeLilypad(GrimPlayer player, InteractionHand hand, int sequence) {
      HitData data = WorldRayTrace.getNearestBlockHitResult(player, (StateType)null, true, false, true);
      if (data != null) {
         if (player.compensatedWorld.getFluidLevelAt(data.position().getX(), data.position().getY() + 1, data.position().getZ()) > 0.0D) {
            return;
         }

         BlockPlace blockPlace = new BlockPlace(player, hand, data.position(), data.closestDirection().getFaceValue(), data.closestDirection(), ItemStack.EMPTY, data, sequence);
         blockPlace.replaceClicked = false;
         if (player.compensatedWorld.getWaterFluidLevelAt(data.position().getX(), data.position().getY(), data.position().getZ()) > 0.0D || data.state().getType() == StateTypes.ICE || data.state().getType() == StateTypes.FROSTED_ICE) {
         ...
         if (event.getConnectionState() != ConnectionState.PLAY) {
            ...
            if (WrapperPlayClientPlayerFlying.isFlying(event.getPacketType())) {
               ...
               if (flying.hasRotationChanged() && !flying.hasPositionChanged() && !flying.isOnGround() && !flying.isHorizontalCollision()) {
                  ...
                  Iterator var7 = player.pendingRotations.iterator();

                  RotationData data;
                  while(var7.hasNext()) {
                     data = (RotationData)var7.next();
                     rotations.add(data);
                     if (!data.isAccepted()) {
                        break;
                     }
            ...
            if (!event.isCancelled() || !WrapperPlayClientPlayerFlying.isFlying(event.getPacketType()) && event.getPacketType() != PacketType.Play.Client.VEHICLE_MOVE) {
               if (WrapperPlayClientPlayerFlying.isFlying(event.getPacketType())) {
                  ...
                  handleFlying(player, pos.getX(), pos.getY(), pos.getZ(), ignoreRotation ? 0.0F : pos.getYaw(), ignoreRotation ? 0.0F : pos.getPitch(), flying.hasPositionChanged(), flying.hasRotationChanged() && !ignoreRotation, flying.isOnGround(), teleportData, event);
               }
               ...
               if (event.getPacketType() == PacketType.Play.Client.PLAYER_BLOCK_PLACEMENT) {
                  ...
                  if (packet.getFace() == BlockFace.OTHER && PacketEvents.getAPI().getServerManager().getVersion().isOlderThan(ServerVersion.V_1_9)) {
                     ...
                     BlockPlace blockPlace = new BlockPlace(player, packet.getHand(), packet.getBlockPosition(), packet.getFaceId(), packet.getFace(), placedWith, WorldRayTrace.getNearestBlockHitResult(player, (StateType)null, true, false, false), packet.getSequence());
                     ...
                     if (!event.isCancelled() && !blockPlace.isCancelled() && !player.getSetbackTeleportUtil().shouldBlockMovement()) {
                        ...
                        if (player.platformPlayer != null) {
                           ...
                           if (packet.getHand() == InteractionHand.MAIN_HAND) {
                              mainHand = player.platformPlayer.getInventory().getItemInHand();
                              player.user.sendPacket((PacketWrapper)(new WrapperPlayServerSetSlot(0, player.inventory.stateID, 36 + player.packetStateData.lastSlotSelected, mainHand)));
                           } else {
                              mainHand = player.platformPlayer.getInventory().getItemInOffHand();
                              player.user.sendPacket((PacketWrapper)(new WrapperPlayServerSetSlot(0, player.inventory.stateID, 45, mainHand)));
                           }
                        }
                     }
                  }
   ...
   private static void handleFlying(GrimPlayer player, double x, double y, double z, float yaw, float pitch, boolean hasPosition, boolean hasLook, boolean onGround, TeleportAcceptData teleportData, PacketReceiveEvent event) {
      ...
      if (!hasPosition && onGround != player.packetStateData.packetPlayerOnGround && !player.inVehicle()) {
         ...
         if ((canFeasiblyPointThree || player.compensatedWorld.isNearHardEntity(player.boundingBox.copy().expand(4.0D))) && (!(player.clientVelocity.getY() > 0.06D) || player.uncertaintyHandler.wasAffectedByStuckSpeed())) {
      ...
      if (hasLook) {
         ...
         RotationUpdate update = new RotationUpdate(new HeadRotation(player.lastYaw, player.lastPitch), new HeadRotation(player.yaw, player.pitch), deltaXRot, deltaYRot);
