...
import ac.grim.grimac.shaded.zaxxer.hikari.HikariCredentialsProvider;
...
import ac.grim.grimac.shaded.zaxxer.hikari.util.Credentials;
...
abstract class PoolBase {
   ...
   private static final int MINIMUM_LOGIN_TIMEOUT = Integer.getInteger("ac.grim.grimac.shaded.zaxxer.hikari.minimumLoginTimeoutSecs", 1);
   private static final boolean LEGACY_USERPASS_DS_OVERRIDE = Boolean.getBoolean("ac.grim.grimac.shaded.zaxxer.hikari.legacy.supportUserPassDataSourceOverride");
   ...
   void quietlyCloseConnection(Connection connection, String closureReason) {
      if (connection != null) {
         try {
            ...
            this.logger.debug("{} - Closing connection {} failed", this.poolName, connection, var9);
         }
      }
      ...
      if (this.config.isRegisterMbeans()) {
         try {
            ...
            if ("true".equals(System.getProperty("hikaricp.jmx.register2.0"))) {
               beanConfigName = new ObjectName("ac.grim.grimac.shaded.zaxxer.hikari:type=PoolConfig,name=" + this.poolName);
               beanPoolName = new ObjectName("ac.grim.grimac.shaded.zaxxer.hikari:type=Pool,name=" + this.poolName);
            } else {
               beanConfigName = new ObjectName("ac.grim.grimac.shaded.zaxxer.hikari:type=PoolConfig (" + this.poolName + ")");
               beanPoolName = new ObjectName("ac.grim.grimac.shaded.zaxxer.hikari:type=Pool (" + this.poolName + ")");
            }
   ...
   private Connection newConnection(boolean isEmptyPool) throws Exception {
      ...
      try {
         ...
         this.logger.debug((String)"{} - Attempting to create/setup new connection ({})", (Object)this.poolName, (Object)id);
         ...
         this.logger.debug((String)"{} - Established new connection ({})", (Object)this.poolName, (Object)id);
         var9 = connection;
      } catch (Throwable var13) {
         this.logger.debug("{} - Failed to create/setup connection ({}): {}", this.poolName, id, var13.getMessage());
         ...
         if (isEmptyPool && ClockSource.elapsedMillis(this.connectionFailureTimestamp.get()) > TimeUnit.MINUTES.toMillis(1L)) {
            this.logger.warn("{} - Pool is empty, failed to create/setup connection ({})", this.poolName, id, var13);
            this.connectionFailureTimestamp.set(0L);
         }
