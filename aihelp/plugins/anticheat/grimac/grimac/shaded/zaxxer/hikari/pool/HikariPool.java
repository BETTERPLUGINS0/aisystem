...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, ConcurrentBag.IBagStateListener {
   ...
   public HikariPool(HikariConfig config) {
      super(config);
      this.aliveBypassWindowMs = Long.getLong("ac.grim.grimac.shaded.zaxxer.hikari.aliveBypassWindowMs", TimeUnit.MILLISECONDS.toMillis(500L));
      this.housekeepingPeriodMs = Long.getLong("ac.grim.grimac.shaded.zaxxer.hikari.housekeeping.periodMs", TimeUnit.SECONDS.toMillis(30L));
      this.lifeTimeVarianceFactor = Math.min(40L, Math.max(2L, Long.getLong("ac.grim.grimac.shaded.zaxxer.hikari.lifeTimeVarianceFactor", 4L)));
      this.isRequestBoundariesEnabled = Boolean.getBoolean("ac.grim.grimac.shaded.zaxxer.hikari.enableRequestBoundaries");
      ...
      if (Boolean.getBoolean("ac.grim.grimac.shaded.zaxxer.hikari.blockUntilFilled") && config.getInitializationFailTimeout() > 1L) {
               ...
               if (!poolEntry.isMarkedEvicted() && (ClockSource.elapsedMillis(poolEntry.lastAccessed, now) <= this.aliveBypassWindowMs || !this.isConnectionDead(poolEntry.connection))) {
                  ...
                  if (this.isRequestBoundariesEnabled) {
                     try {
                        ...
                        this.logger.warn((String)"beginRequest Failed for: {}, ({})", (Object)poolEntry.connection, (Object)var15.getMessage());
                     }
                  }
                  break label109;
               }
   ...
   public void setMetricRegistry(Object metricRegistry) {
      if (metricRegistry != null && UtilityElf.safeIsAssignableFrom(metricRegistry, "com.codahale.metrics.MetricRegistry")) {
         this.setMetricsTrackerFactory((MetricsTrackerFactory)UtilityElf.createInstance("ac.grim.grimac.shaded.zaxxer.hikari.metrics.dropwizard.CodahaleMetricsTrackerFactory", MetricsTrackerFactory.class, metricRegistry));
      } else if (metricRegistry != null && UtilityElf.safeIsAssignableFrom(metricRegistry, "io.dropwizard.metrics5.MetricRegistry")) {
         this.setMetricsTrackerFactory((MetricsTrackerFactory)UtilityElf.createInstance("ac.grim.grimac.shaded.zaxxer.hikari.metrics.dropwizard.Dropwizard5MetricsTrackerFactory", MetricsTrackerFactory.class, metricRegistry));
      } else if (metricRegistry != null && UtilityElf.safeIsAssignableFrom(metricRegistry, "io.micrometer.core.instrument.MeterRegistry")) {
         this.setMetricsTrackerFactory((MetricsTrackerFactory)UtilityElf.createInstance("ac.grim.grimac.shaded.zaxxer.hikari.metrics.micrometer.MicrometerMetricsTrackerFactory", MetricsTrackerFactory.class, metricRegistry));
      } else {
   ...
   public void setHealthCheckRegistry(Object healthCheckRegistry) {
      if (healthCheckRegistry != null) {
         try {
            Class<?> checkerClazz = HikariPool.class.getClassLoader().loadClass("ac.grim.grimac.shaded.zaxxer.hikari.metrics.dropwizard.CodahaleHealthChecker");
      ...
      if (poolEntry.isMarkedEvicted()) {
         ...
         if (this.isRequestBoundariesEnabled) {
            try {
               ...
               this.logger.warn((String)"endRequest Failed for: {},({})", (Object)poolEntry.connection, (Object)var3.getMessage());
            }
         }
   ...
   private PoolEntry createPoolEntry() {
      try {
         ...
         if (this.poolState == 0) {
            this.logger.debug((String)"{} - Error thrown while acquiring connection from data source", (Object)this.poolName, (Object)var10.getCause());
         }
   ...
   private synchronized void fillPool(boolean isAfterAdd) {
      ...
      if (shouldAdd) {
         ...
         this.logger.debug((String)"{} - Fill pool skipped, pool has sufficient level or currently being filled.", (Object)this.poolName);
      }
   ...
   private final class HouseKeeper implements Runnable {
      ...
      public void run() {
         try {
            ...
            HikariPool.this.logger.error((String)"Unexpected exception in housekeeping task", (Throwable)var9);
         }
