...
import ac.grim.grimac.shaded.slf4j.helpers.NOP_FallbackServiceProvider;
...
public final class LoggerFactory {
   ...
   private static final String STATIC_LOGGER_BINDER_PATH = "ac/grim/grimac/shaded/slf4j/impl/StaticLoggerBinder.class";
   ...
   static SLF4JServiceProvider loadExplicitlySpecified(ClassLoader classLoader) {
      ...
      if (null != explicitlySpecified && !explicitlySpecified.isEmpty()) {
         ...
         try {
            ...
            message = String.format("Failed to instantiate the specified SLF4JServiceProvider (%s)", explicitlySpecified);
            Reporter.error(message, var6);
            return null;
         } catch (ClassCastException var7) {
            message = String.format("Specified SLF4JServiceProvider (%s) does not implement SLF4JServiceProvider interface", explicitlySpecified);
            Reporter.error(message, var7);
            return null;
         }
      } else {
         return null;
   ...
   private static void reportIgnoredStaticLoggerBinders(Set<URL> staticLoggerBinderPathSet) {
      if (!staticLoggerBinderPathSet.isEmpty()) {
         ...
         Iterator var1 = staticLoggerBinderPathSet.iterator();
   ...
   static Set<URL> findPossibleStaticLoggerBinderPathSet() {
      ...
      try {
         ...
         if (loggerFactoryClassLoader == null) {
            paths = ClassLoader.getSystemResources("ac/grim/grimac/shaded/slf4j/impl/StaticLoggerBinder.class");
         } else {
            paths = loggerFactoryClassLoader.getResources("ac/grim/grimac/shaded/slf4j/impl/StaticLoggerBinder.class");
         }
   ...
   private static void fixSubstituteLoggers() {
      synchronized(SUBST_PROVIDER) {
         ...
         Iterator var1 = SUBST_PROVIDER.getSubstituteLoggerFactory().getLoggers().iterator();
   ...
   private static void reportMultipleBindingAmbiguity(List<SLF4JServiceProvider> providerList) {
      if (isAmbiguousProviderList(providerList)) {
         ...
         Iterator var1 = providerList.iterator();

         while(var1.hasNext()) {
            SLF4JServiceProvider provider = (SLF4JServiceProvider)var1.next();
