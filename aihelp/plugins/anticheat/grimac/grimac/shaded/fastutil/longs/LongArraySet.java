...
public class LongArraySet extends AbstractLongSet implements Serializable, Cloneable {
      ...
      return new LongIterator() {
         ...
         public long nextLong() {
            if (!this.hasNext()) {
               ...
               return LongArraySet.this.a[this.curr = this.next++];
            }
         }
         ...
         public int skip(int n) {
            if (n < 0) {
               ...
               n = Math.min(n, LongArraySet.this.size - this.next);
               this.next += n;
               if (n != 0) {
         ...
         public void forEachRemaining(java.util.function.LongConsumer action) {
            long[] a = LongArraySet.this.a;

            while(this.next < LongArraySet.this.size) {
   ...
   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
      s.defaultReadObject();
      long[] a = this.a = new long[this.size];
