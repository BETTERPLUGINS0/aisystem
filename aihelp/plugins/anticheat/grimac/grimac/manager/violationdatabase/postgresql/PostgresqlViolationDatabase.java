...
public class PostgresqlViolationDatabase implements ViolationDatabase {
   ...
   public void connect() throws SQLException {
      try {
         ...
         LogUtil.error("Failed to generate violations database:", var6);
         throw var6;
      }
   }
   ...
   public synchronized void logAlert(GrimPlayer player, String grimVersion, String verbose, String checkName, int vls) {
      try {
         ...
         try {
            PreparedStatement insertAlert = connection.prepareStatement("INSERT INTO grim_history_violations (server_id, uuid, check_name_id, " + quoteVerboseColumn() + ", vl, created_at, grim_version_id, client_brand_id, client_version_id, server_version_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
         ...
         LogUtil.error("Failed to log alert", var25);
      }
   ...
   public synchronized int getLogCount(UUID uuid) {
      try {
         ...
         LogUtil.error("Failed to count logs", var10);
      }
   ...
   public synchronized List<Violation> getViolations(UUID uuid, int page, int limit) {
      try {
         ...
         LogUtil.error("Failed to fetch logs", var12);
         return null;
      }
   }
