...
public final class BlockCollisions {
   ...
   public static final ImmutableList<Collisions.Axis> YXZ_AXIS_ORDER;
   public static final ImmutableList<Collisions.Axis> YZX_AXIS_ORDER;
   ...
   public static Vector3i getFurthestCorner(Vector3d vector) {
      ...
      int xSign = vector.x >= 0.0D ? 1 : -1;
      int ySign = vector.y >= 0.0D ? 1 : -1;
      int zSign = vector.z >= 0.0D ? 1 : -1;
   ...
   public static Optional<Vector3d> clip(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, Vector3d start, Vector3d end) {
      double[] minDistance = new double[]{1.0D};
      double deltaX = end.x - start.x;
      double deltaY = end.y - start.y;
      double deltaZ = end.z - start.z;
      Direction direction = getDirection(minX, minY, minZ, maxX, maxY, maxZ, start, minDistance, (Direction)null, deltaX, deltaY, deltaZ);
      if (direction == null) {
   ...
   public static Direction clipPoint(double[] minDistance, Direction prevDirection, double distanceSide, double distanceOtherA, double distanceOtherB, double minSide, double minOtherA, double maxOtherA, double minOtherB, double maxOtherB, Direction hitSide, double startSide, double startOtherA, double startOtherB) {
      ...
      if (sideDistance > 0.0D && sideDistance < minDistance[0] && minOtherA - 1.0E-7D < otherDistanceA && otherDistanceA < maxOtherA + 1.0E-7D && minOtherB - 1.0E-7D < otherDistanceB && otherDistanceB < maxOtherB + 1.0E-7D) {
   ...
   static {
      YXZ_AXIS_ORDER = ImmutableList.of(Collisions.Axis.Y, Collisions.Axis.X, Collisions.Axis.Z);
      YZX_AXIS_ORDER = ImmutableList.of(Collisions.Axis.Y, Collisions.Axis.Z, Collisions.Axis.X);
   }
}
