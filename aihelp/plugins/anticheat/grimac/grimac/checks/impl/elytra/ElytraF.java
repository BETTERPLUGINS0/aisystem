...
public class ElytraF extends Check implements PostPredictionCheck {
   ...
   public void onPacketReceive(PacketReceiveEvent event) {
      if (!this.player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_8)) {
         if (event.getPacketType() == PacketType.Play.Client.ENTITY_ACTION && (new WrapperPlayClientEntityAction(event)).getAction() == WrapperPlayClientEntityAction.Action.START_FLYING_WITH_ELYTRA && this.player.clientClaimsLastOnGround && this.flagAndAlert()) {
            this.setback = true;
